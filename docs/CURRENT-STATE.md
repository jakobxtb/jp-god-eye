...
- Repository metadata and public URLs use the `JakobPapaj/jp-god-eye`
...
| Mapped Installations ⌖ | OpenStreetMap mapped context (the Google Places supplement was removed with the free-tier migration; Overpass was already the primary source, so re-asking it under a second name added cost and no information) | `src/data/militaryInstallations.js` | `/api/military-installations` | viewport-driven + user search; while unavailable, auto-retry 30 s → 240 s backoff |
...
- **Scene context** (`get_entity_context`): selected entity from the context store, or visible entities ranked by distance to the view target (≤100km altitude); plus basemap context — view-target picking, 7-point viewport sampling, view-scale classification (global/continental/regional/metro/city/local), reverse geocoding via `/api/reverse-geocode` (Nominatim; center ≤750km, viewport samples ≤3,000km), nearby named-landmark context via `/api/places/nearby` (Overpass; ≤25km), known-landmark matching against `CITY_POIS`, coarse country inference fallback. Context assembly is capped at 1.5s with cache fallbacks; caches are deduped in-flight.
...
- `/api/geocode` forward-geocodes through Nominatim (Photon as a real secondary), with a shared ≥1.1 s OSM queue, a 7-day cache and in-flight coalescing. `bounded=1` makes it the view-BOUNDED landmark search that replaced Google Places text search for annotation resolution.
- `/api/reverse-geocode` reverse-geocodes through Nominatim on the same queue, cached per ~10 m coordinate cell.
- `/api/places/nearby` returns nearby named landmarks from Overpass, reusing the hardened Overpass pipeline (mirror rotation, disk cache, serve-stale). Replaced the Google Places Nearby proxy; same response contract.
