...
JP GOD EYE is a local-first client for **public** data. It is built for exploration, demos, and learning — not as a hardened production service. This document explains the security model so you can run it safely and report issues responsibly.

## Reporting a vulnerability

Please report security issues **privately** — do not open a public issue for anything exploitable.

- Use GitHub's [private vulnerability reporting](https://github.com/JakobPapaj/jp-god-eye/security/advisories/new) (Security tab → "Report a vulnerability"), or
...
- **App-level throttles (opt-in):** `GEV_RATELIMIT_OPENAI_PER_MIN` caps the remaining cost-bearing endpoints per client IP per minute (over-limit requests receive a sanitized `429`). It is a **per-IP, process-local, in-memory guard** — it resets on restart and is **not a billing cap**. The former `GEV_RATELIMIT_GOOGLE_PER_MIN` is gone along with the Google Places endpoints it guarded; geocoding and place search now run through `/api/geocode`, `/api/reverse-geocode` and `/api/places/nearby`, which are rate-limited unconditionally to protect the shared free OpenStreetMap services rather than a bill.
