...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    fetch('/osiris/api/cloudflare-radar?probe=1')
  ...
  const handleRightClick = useCallback(async (coords: { lat: number; lng: number }) => {
    ...
    try {
      const res = await fetch(`/osiris/api/region-dossier?lat=${coords.lat}&lng=${coords.lng}`);
  ...
  useEffect(() => {
    ...
    const loadMarkets = async (attempt = 0) => {
      await fetchEndpoint('/osiris/api/markets', d => ({ markets: d }));
    ...
    const spaceTimer = setTimeout(async () => {
      try {
        const r = await fetch('/osiris/api/space-weather');
    ...
    const intervals = [
      ...
      setInterval(() => fetchEndpoint('/osiris/api/news', undefined, undefined, { skipWhenHidden: true }), 1800000),        // 30 min (was 10)
      setInterval(() => fetchEndpoint('/osiris/api/markets', d => ({ markets: d }), undefined, { skipWhenHidden: true }), 900000), // 15 min (was 5)
    ];
    return () => {
  ...
  useEffect(() => {
    ...
    if (activeLayers.flights || activeLayers.military || activeLayers.jets || activeLayers.private) {
      if (!layerFetchedRef.current.has('flights')) {
        fetchEndpoint('/osiris/api/flights');
    ...
    if (activeLayers.global_incidents && !layerFetchedRef.current.has('gdelt')) {
      fetchEndpoint('/osiris/api/gdelt', d => ({ gdelt: d.events }));
    ...
    if (activeLayers.malware && !layerFetchedRef.current.has('malware')) {
      fetchEndpoint('/osiris/api/malware', d => ({ malware_threats: d.threats }));
    ...
    if ((activeLayers as any).cyber_attacks && !layerFetchedRef.current.has('cyber_attacks')) {
      fetchEndpoint('/osiris/api/cyber-attacks', d => ({ cyber_attacks: d.attacks }));
    ...
    if ((activeLayers as any).gdelt_events) {
      loadLayerOnce('gdelt_events', '/osiris/api/gdelt-events?limit=600', d => ({ gdelt_events: d.events }));
    }

    // Cloudflare Radar — one request backs both layers
    if ((activeLayers as any).cf_outages || (activeLayers as any).cf_attacks) {
      loadLayerOnce('cloudflare_radar', '/osiris/api/cloudflare-radar', d => ({
  ...
  useEffect(() => {
    ...
    if (activeLayers.flights || activeLayers.military || activeLayers.jets || activeLayers.private) {
      intervals.push(setInterval(() => fetchEndpoint('/osiris/api/flights'), 300000)); // 5 min (was 2 min)
    }

    if (activeLayers.balloons) {
      intervals.push(setInterval(() => fetchEndpoint('/osiris/api/balloons', d => ({ balloons: d.balloons })), 300000)); // 5m
    }
    if (activeLayers.radiation) {
      intervals.push(setInterval(() => fetchEndpoint('/osiris/api/radiation', d => ({ radiation: d.stations })), 300000)); // 5m
    }
    if (activeLayers.maritime) {
      intervals.push(setInterval(() => fetchEndpoint('/osiris/api/maritime', d => ({ maritime_ports: d.ports, maritime_chokepoints: d.chokepoints, maritime_ships: d.ships })), 10000)); // 10s
    }
              ...
              onReroute={async (fromPt) => {
                ...
                try {
                  const res = await fetch(
                    `/osiris/api/directions?from=${fromPt.lat},${fromPt.lng}&to=${dest[1]},${dest[0]}&mode=auto`,
                  );
