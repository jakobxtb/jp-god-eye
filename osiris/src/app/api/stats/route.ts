...
export async function GET(req: Request) {
  try {
    ...
    const [flightsRes, satsRes, cctvRes, weatherRes, infraRes, gdeltRes] = await Promise.allSettled([
      fetch(`${origin}/osiris/api/flights`, { signal: AbortSignal.timeout(20000), next: { revalidate: 45 } }),
      fetch(`${origin}/osiris/api/satellites`, { signal: AbortSignal.timeout(20000), next: { revalidate: 3600 } }),
      fetch(`${origin}/osiris/api/cctv`, { signal: AbortSignal.timeout(20000), next: { revalidate: 3600 } }),
      fetch(`${origin}/osiris/api/weather`, { signal: AbortSignal.timeout(20000), next: { revalidate: 300 } }),
      fetch(`${origin}/osiris/api/infrastructure`, { signal: AbortSignal.timeout(20000), next: { revalidate: 86400 } }),
      fetch(`${origin}/osiris/api/gdelt`, { signal: AbortSignal.timeout(20000), next: { revalidate: 300 } })
    ]);
