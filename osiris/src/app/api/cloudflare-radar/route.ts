...
export async function GET(req: Request) {
  ...
  const [outagesRes, attacksRes] = await Promise.allSettled([
    // Both Radar endpoints REQUIRE a time window — without `dateRange` (or an
    // explicit start/end pair) the API answers 400 "You must send either range
    // or start & end dates", which is what made this route report the layers
    // as unavailable even with a valid token. 7d matches what the map layers
    // present: recent, still-relevant events rather than a live snapshot.
    radarFetch('/annotations/outages?dateRange=7d&limit=50&format=json', signal),
    radarFetch('/attacks/layer3/top/locations/origin?dateRange=7d&limit=25&format=json', signal),
  ]);
