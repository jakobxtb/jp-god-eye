        ...
        <main ref={mainRef} className="flex-1 min-w-0 py-12 lg:py-16 max-w-3xl">
          ...
          <Section id="quickstart" eyebrow="Guide" title="Quick Start">
            ...
            <CodeBlock
              ...
              tabs={[
                { label: 'cURL', lang: 'bash', code: `curl -s ${origin}/osiris/api/flights | jq '.commercial_flights | length'` },
                {
                  label: 'JavaScript',
                  lang: 'javascript',
                  code: `const res = await fetch("${origin}/osiris/api/flights");
...
data = requests.get("${origin}/osiris/api/flights").json()
print(len(data["commercial_flights"]), "commercial")`,
            ...
            <Pre label="Aggregate counters" lang="bash">{`curl -s ${origin}/osiris/api/stats
...
#   "timestamp": "2026-07-29T12:00:00Z" }`}</Pre>
            ...
            <Pre label="Passive subdomain enumeration" lang="bash">{`curl -s "${origin}/osiris/api/osint/certs?domain=example.com" | jq -r '.subdomains[]'`}</Pre>
