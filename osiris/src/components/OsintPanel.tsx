...
function OsintPanelInner({ isMobile, onSweepVisualize, onScanGeolocate }: OsintPanelProps) {
  ...
  const fetchCveDetails = useCallback(async (cveIds: string[]) => {
    ...
    const results = await Promise.allSettled(
      missing.map(id => fetch(`/osiris/api/osint/cve?cve=${encodeURIComponent(id)}`).then(r => r.json()).then(data => ({ id, data })))
    );
    setCveCache(prev => {
      const next = { ...prev };
  ...
  const runLookup = useCallback(async () => {
    ...
    if (activeTab === 'sweep') {
      ...
      try {
        const t0 = Date.now();
        const res = await fetch(`/osiris/api/osint/sweep?ip=${encodeURIComponent(query)}&cidr=${cidr}`);
    ...
    try {
      ...
      switch (activeTab) {
        ...
        case 'dns': url = `/osiris/api/osint/dns?domain=${encodeURIComponent(query)}`; break;
        case 'certs': url = `/osiris/api/osint/certs?domain=${encodeURIComponent(query)}`; break;
        case 'whois': url = `/osiris/api/osint/whois?domain=${encodeURIComponent(query)}`; break;
        case 'threats': url = `/osiris/api/osint/threats?query=${encodeURIComponent(query)}`; break;
        case 'bgp': url = `/osiris/api/osint/bgp?query=${encodeURIComponent(query)}`; break;
        case 'mac': url = `/osiris/api/osint/mac?mac=${encodeURIComponent(query)}`; break;
        case 'phone': url = `/osiris/api/osint/phone?number=${encodeURIComponent(query)}`; break;
        case 'leaks': url = `https://api.xposedornot.com/v1/breach-analytics?email=${encodeURIComponent(query)}`; break;
        case 'infostealer': url = `/osiris/api/osint/hudsonrock?query=${encodeURIComponent(query)}`; break;
        case 'crypto': url = `/osiris/api/osint/crypto?address=${encodeURIComponent(query)}`; break;
        case 'username': url = `/osiris/api/osint/username?username=${encodeURIComponent(query)}`; break;
        case 'github': url = `/osiris/api/osint/github?user=${encodeURIComponent(query)}`; break;
        case 'vuln': url = `/osiris/api/scanner?target=${encodeURIComponent(query)}&type=vuln`; break;
        case 'scanner': url = `/osiris/api/scanner?target=${encodeURIComponent(query)}&type=${scanType}`; break;
        case 'headers': url = `/osiris/api/scanner?target=${encodeURIComponent(query)}&type=headers`; break;
        case 'ssl': url = `/osiris/api/scanner?target=${encodeURIComponent(query)}&type=ssl`; break;
        case 'subdomains': url = `/osiris/api/scanner?target=${encodeURIComponent(query)}&type=subdomains`; break;
        case 'tech': url = `/osiris/api/scanner?target=${encodeURIComponent(query)}&type=tech`; break;
      ...
      if (res.ok) {
        ...
        if (activeTab === 'phone') {
          ...
          fetch(`/osiris/api/osint/ip?ip=${encodeURIComponent(query)}`)
            .then(r => r.json())
            .then(locData => {
