    ...
    timer.current = setTimeout(async () => {
      ...
      try {
        ...
        const res = await fetch(
          `/osiris/api/geosearch?q=${encodeURIComponent(q)}${bias}`,
          { signal: ctrl.signal },
        );
...
export default function DirectionsBar({ onRoute, onLocate, onClose, center = null, onLiveLocation, onActiveSegment, onFollowChange, onStartNavigation }: DirectionsBarProps) {
  ...
  const runRoute = useCallback(async (
    ...
    try {
      ...
      const res = await fetch(
        `/osiris/api/directions?from=${a.lat},${a.lng}&to=${b.lat},${b.lng}&mode=${m}${viaParam}${avoidParam}`,
      );
  ...
  const useMyLocation = useCallback(async () => {
    ...
    try {
      const res = await fetch('/osiris/api/geo');
