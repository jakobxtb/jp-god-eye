...
async function collectEthereum(address: string): Promise<RawWallet> {
  ...
  // Internal transactions: value moved BY CONTRACTS rather than by a signed
  // top-level transaction. Blockscout's normal tx list omits them entirely, so
  // an address funded through a router, a proxy or a self-destruct looks
  // emptier than it is. Etherscan exposes them, and only with a key.
  const sources = ['Blockscout'];
  const etherscanKey = String(process.env.ETHERSCAN_API_KEY || '').trim();
  if (etherscanKey) {
    try {
      // V2 endpoint with an explicit chainid — the V1 host now answers every
      // request with a deprecation notice instead of data.
      const resp = await fetch(
        `https://api.etherscan.io/v2/api?chainid=1&module=account&action=txlistinternal`
          + `&address=${encodeURIComponent(address)}&page=1&offset=${TX_PAGE}&sort=desc`
          + `&apikey=${encodeURIComponent(etherscanKey)}`,
        { signal: AbortSignal.timeout(20000) },
      );
      if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
      const payload = await resp.json();
      // Etherscan answers 200 with status '0' for "no records", which is a
      // legitimate empty result, not a failure.
      if (payload?.status === '1' && Array.isArray(payload.result)) {
        const lower = address.toLowerCase();
        const internals: TxSummary[] = payload.result.map((t: any): TxSummary => {
          const isIn = String(t?.to || '').toLowerCase() === lower;
          return {
            hash: String(t?.hash || ''),
            time: t?.timeStamp ? new Date(Number(t.timeStamp) * 1000).toISOString() : null,
            direction: isIn ? 'in' : 'out',
            value: Number(t?.value || 0) / 1e18,
            counterparty: (isIn ? t?.from : t?.to) || null,
            failed: String(t?.isError || '0') === '1',
          };
        });
        // Merge without duplicating a hash Blockscout already reported.
        const seen = new Set(transactions.map((t) => t.hash));
        const added = internals.filter((t) => t.hash && !seen.has(t.hash));
        if (added.length) {
          transactions = [...transactions, ...added]
            .sort((a, b) => String(b.time || '').localeCompare(String(a.time || '')));
          if (flow) {
            flow.total_in += added.filter((t) => t.direction === 'in').reduce((s, t) => s + t.value, 0);
            flow.total_out += added.filter((t) => t.direction === 'out').reduce((s, t) => s + t.value, 0);
            flow.net = flow.total_in - flow.total_out;
          }
        }
        sources.push('Etherscan (internal txs)');
      } else if (payload?.status !== '0') {
        throw new Error(String(payload?.result || payload?.message || 'unexpected payload'));
      }
    } catch (e) {
      partial.push(`internal transactions unavailable (${e instanceof Error ? e.message : 'error'})`);
    }
  } else {
    partial.push('Contract-internal value transfers are not included (set ETHERSCAN_API_KEY)');
  }
...
/**
 * Turn one Helius enhanced-transaction record into a TxSummary.
 *
 * Helius pre-parses the transfer legs the raw RPC only exposes as opaque
 * instruction data, so direction, amount and counterparty become answerable
 * without a per-transaction fetch. Only NATIVE SOL legs touching this address
 * are summarised — token transfers are reported separately by the caller.
 */
function summarizeHeliusTx(tx: any, address: string): TxSummary {
  const legs: any[] = Array.isArray(tx?.nativeTransfers) ? tx.nativeTransfers : [];
  const mine = legs.filter(
    (l) => l?.fromUserAccount === address || l?.toUserAccount === address,
  );
  // Net the legs: a single transaction can both credit and debit the address
  // (a swap routed through it), and only the net movement is meaningful.
  let netLamports = 0;
  let counterparty: string | null = null;
  for (const leg of mine) {
    const amount = Number(leg?.amount) || 0;
    if (leg.toUserAccount === address) {
      netLamports += amount;
      counterparty = counterparty ?? leg.fromUserAccount ?? null;
    } else {
      netLamports -= amount;
      counterparty = counterparty ?? leg.toUserAccount ?? null;
    }
  }
  const direction: TxSummary['direction'] =
    netLamports > 0 ? 'in' : netLamports < 0 ? 'out' : 'unknown';
  return {
    hash: String(tx?.signature || ''),
    time: tx?.timestamp ? new Date(tx.timestamp * 1000).toISOString() : null,
    direction,
    value: Math.abs(netLamports) / 1e9,
    counterparty,
    failed: !!tx?.transactionError,
  };
}

async function collectSolana(address: string): Promise<RawWallet> {
  const heliusKey = String(process.env.HELIUS_API_KEY || '').trim();
  // A keyed RPC is not just faster — the public endpoint rate-limits the very
  // calls this function depends on, so the key changes what is answerable.
  const url = heliusKey
  ...
  const partial: string[] = [];
  let transactions: TxSummary[] = [];
  // Provenance must name the source that actually answered, so a reader can
  // tell a parsed-transfer history from a bare signature listing.
  let usedHelius = false;

  if (heliusKey) {
    // Enhanced transactions: parsed transfer legs, so amounts, direction and
    // counterparties are real values rather than 'unknown'.
    try {
      const resp = await fetch(
        `https://api.helius.xyz/v0/addresses/${encodeURIComponent(address)}/transactions?api-key=${heliusKey}&limit=50`,
        { signal: AbortSignal.timeout(20000) },
      );
      if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
      const rows = await resp.json();
      if (!Array.isArray(rows)) throw new Error('unexpected payload');
      transactions = rows.map((tx) => summarizeHeliusTx(tx, address));
      usedHelius = true;
    } catch (e) {
      partial.push(
        `Helius enhanced history unavailable (${e instanceof Error ? e.message : 'error'}); falling back to signatures`,
      );
    }
  }
  ...
  return {
    ...
    tokens,
    sources: usedHelius
      ? ['Helius RPC', 'Helius enhanced transactions']
      : ['Solana JSON-RPC'],
    partial,
  };
}
