...
JP GOD EYE's **code** is [MIT](LICENSE)-licensed. **The MIT grant covers the source code only — it does NOT extend to third-party data or visual assets.** Every third-party source keeps its own license and terms. This file documents the live and bundled data sources; bundled 3D-model provenance is recorded in [`public/models/README.md`](public/models/README.md).
...
|--------|----------|-----------------|-------------|
| **EOX Sentinel-2 cloudless 2025** (WMTS) | Default keyless satellite basemap for the `satellite` map stack (10 m) | **CC BY-NC-SA 4.0 — NonCommercial** | "Sentinel-2 cloudless 2025 by EOX IT Services GmbH (Contains modified Copernicus Sentinel data 2025)" with links, per the service's own capabilities document |
| **NASA EOSDIS GIBS** (Blue Marble shaded relief + bathymetry, WMTS) | Keyless satellite basemap fallback, and the commercial-safe option (`GEV_KEYLESS_IMAGERY=gibs`) | U.S. public domain, no restriction | "Imagery courtesy of NASA EOSDIS GIBS" (courtesy) |
| **Cesium ion** (Bing world imagery, Cesium World Terrain, **Cesium OSM Buildings** asset 96188) | Bing map stacks, world terrain, and global 3D building massing | Cesium ion terms; the free tier is **individual non-commercial** (5 GB storage / 15 GB streaming per month). Buildings are OSM-derived | Cesium ion credit (rendered by Cesium) + "© OpenStreetMap contributors" (ODbL 1.0) |
| **OpenStreetMap Nominatim** | Forward geocoding (location search, voice fly-to, annotation targeting) and reverse geocoding | ODbL 1.0 + [Nominatim usage policy](https://operations.osmfoundation.org/policies/nominatim/) — max 1 req/s, identifying User-Agent, cache results | "© OpenStreetMap contributors" |
| **Photon** (komoot) | Forward-geocoding fallback when Nominatim fails or returns nothing | ODbL 1.0 (OSM-derived) | "© OpenStreetMap contributors" |
| **OpenStreetMap (Overpass API)** | Nearby named-landmark context (the scene/voice place labels that replaced Google Places) | ODbL 1.0 | "© OpenStreetMap contributors" |
| **Mapillary** | Optional street-level fallback frame for a CCTV camera whose own upstream feed is down | CC BY-SA 4.0; free developer token | "Mapillary contributors (CC BY-SA 4.0)" |
| **Google Map Tiles API** (Photorealistic 3D Tiles) — **OPTIONAL, OFF BY DEFAULT** | The Google 3D map stack only, when `GOOGLE_MAPS_API_KEY` is explicitly set | Google Maps Platform ToS (proprietary, your own key + billing; Enterprise SKU) | "Google" / "Google Maps" logo — **shown in-app**, required whenever the stack is active |
...
- **Google Maps Platform — optional and off by default.** Nothing in the shipped configuration calls Google. Setting `GOOGLE_MAPS_API_KEY` enables exactly one thing: the `Google 3D` map stack. Geocoding, reverse geocoding, place search and the CCTV fallback frame were all migrated off Google and now run on OpenStreetMap and Mapillary. If you do enable the stack you supply your own key, need a billing account (Photorealistic 3D Tiles is an Enterprise SKU with ~1,000 free events/month, not covered by the subscription plans), and are bound by [Google's ToS](https://cloud.google.com/maps-platform/terms). Google Maps Content **may not be cached, stored, rehosted, or committed** — the app only ever uses it live. The "Google" attribution is displayed on the globe and must stay visible. Restrict your key (see [SECURITY.md](SECURITY.md)). **Note a residual tension you own, not the app:** Google's terms expect Google's geocoder alongside their tiles, and this build geocodes through Nominatim — Cesium's one-time console warning about that is left visible on purpose.

- **Keyless satellite basemap.** The default `satellite` map stack streams the EOX Sentinel-2 cloudless 2025 mosaic (10 m) straight from EOX's public WMTS — no key, no account. It is **CC BY-NC-SA 4.0, so NonCommercial**: commercial deployments must set `GEV_KEYLESS_IMAGERY=gibs` to pin the public-domain NASA GIBS basemap instead, or license the mosaic from EOX. One cheap probe request per session decides which source is actually reachable, and an EOX outage falls back to GIBS on its own rather than leaving a blank globe. The required EOX attribution renders as an on-globe Cesium credit whenever the mosaic is active.

- **Cesium ion.** Optional and free-tier: a token unlocks Bing world imagery, Cesium World Terrain, and Cesium OSM Buildings (asset 96188), which supplies the global 3D building massing that Google Photorealistic 3D Tiles used to provide. The buildings ride on every globe stack, and are hidden on the Google stack because that mesh already contains its own buildings. **The ion free tier is for individual non-commercial use** with a 15 GB/month streaming allowance — buildings plus imagery consume it, so check your ion dashboard rather than assuming headroom.

- **Geocoding and place search (OpenStreetMap).** `/api/geocode`, `/api/reverse-geocode` and `/api/places/nearby` replaced the Google Geocoding and Places endpoints. Every Nominatim request in the process — search, reverse geocode, and the cockpit regional brief — is serialized behind a single ≥1.1 s queue with an identifying User-Agent, because the OSM policy limit is per client and not per endpoint. Results are cached aggressively (7 days for forward geocodes, 24 h for reverse, 1 h for place search) and concurrent identical lookups are coalesced onto one upstream request. Photon is queried only when Nominatim fails or returns nothing; its ranking is measurably weaker, so it is a fallback and never the primary. Place search runs through the existing hardened Overpass proxy (mirror rotation, disk cache, serve-stale), asking only for a narrow allow-list of named landmark tag families inside a capped radius.
...
> If you use JP GOD EYE commercially, delete `src/data/local_data/telegeography_submarine_cables/` (or obtain a commercial license from TeleGeography). It is one self-contained folder; the rest of the app runs without it.
