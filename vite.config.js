...
import { SETUP_KEYS, validateSetupValue, mergeEnv } from './src/setupKeys.js';
...
import {
  ...
  utcMonthKey as tomtomUtcMonthKey,
...
// The Google Places spend limiter is gone with the endpoints it guarded:
// geocoding and place search now run through the keyless /api/geocode,
// /api/reverse-geocode and /api/places/nearby proxies, which are rate-limited
// to protect the free OSM services rather than an operator's bill.
...
const RADIO_USER_AGENT = 'JpGodEye/1.0 (Radio Browser directory client)';
...
function celestrakProxy() {
  ...
  async function fetchUpstream(group) {
    ...
    const res = await fetch(url.toString(), {
      ...
      headers: { 'User-Agent': 'jp-god-eye-celestrak-proxy/1.0 (+https://github.com/JakobPapaj/jp-god-eye)' },
    });
...
function tomtomProxy() {
  ...
  // TomTom's no-credit-card developer plan grants 200,000 tile requests per
  // MONTH. The default sits ~10% under that so an ordinary month of use can
  // never cross into a billable state, and so a burst still leaves headroom.
  const DEFAULT_MONTHLY_BUDGET = 180000;
  ...
  function monthlyBudgetLimit() {
    const raw = Number.parseInt(process.env.TOMTOM_MONTHLY_TILE_BUDGET || '', 10);
    if (Number.isFinite(raw) && raw > 0) return raw;
    // The retired per-day variable is still honoured so an existing .env keeps
    // governing something sane instead of silently jumping to the new default:
    // its value is read as a daily rate and projected across a 31-day month.
    const legacyDaily = Number.parseInt(process.env.TOMTOM_DAILY_TILE_BUDGET || '', 10);
    if (Number.isFinite(legacyDaily) && legacyDaily > 0) return legacyDaily * 31;
  ...
  /** Roll the counter to the current UTC month and return it. */
  function currentBudget() {
    budget = normalizeTomTomBudget(budget, tomtomUtcMonthKey());
    return budget;
  }

  /** Count one upstream fetch attempt against this month's budget (async persist). */
      ...
      server.middlewares.use('/api/tomtom', async (req, res) => {
        ...
        try {
          ...
          if (urlPath === '/status') {
            ...
            sendJson(200, {
              hasKey,
              // `dailyCount`/`date` keep their names so the existing client
              // status reader keeps working; the values are now the month's
              // running total and the 'YYYY-MM' period they belong to.
              dailyCount: b.count,
              monthlyCount: b.count,
          ...
          if (isTomTomOverBudget(currentBudget(), monthlyBudgetLimit())) {
            if (entry) {
    ...
    try {
      const upstream = await fetch(endpoint, {
        ...
        headers: {
          ...
          'User-Agent': 'jp-god-eye-overpass-proxy/1.0',
        },
        body,
        signal: controller.signal,
      });
        ...
        try {
          ...
          try {
            const upstreamRes = await fetch(upstream, {
              signal: controller.signal,
              headers: { 'User-Agent': 'jp-god-eye/dev (local)' },
            });
    ...
    try {
      const upstream = await fetch(
        `https://api.adsb.lol/v2/lat/${roundedLat}/lon/${roundedLon}/dist/${ADSBLOL_POINT_RADIUS_NM}`,
        {
          headers: {
            Accept: 'application/json',
            'User-Agent': 'jp-god-eye-adsblol-regional-fallback/1.0',
          },
          signal: controller.signal,
        },
      );
        ...
        try {
          ...
          try {
            upstream = await fetch(upstreamUrl.toString(), {
              ...
              headers: {
                Accept: 'application/json',
                'User-Agent': 'jp-god-eye-gbfs-proxy/1.0',
              },
              signal: controller.signal,
            });
          } finally {
 ...
 * this keeps every request small AND spreads cameras across the continent
 * instead of returning 300 cameras from whichever country sorts first.
 * Each entry is [north, east, south, west] as the v3 `bbox` parameter expects.
 */
const WINDY_EUROPE_BOXES = [
  // DACH is deliberately split fine-grained. Windy's `bbox` + popularity sort
  // returns the most-viewed cameras IN the box, so a wide box lets a
  // camera-dense neighbour crowd out a whole country: one combined
  // Austria/Czech/Slovakia box yielded 1 Austrian camera and zero in Vienna.
  // Splitting per-country is what actually maximises European coverage.
  { id: 'at-vienna', bbox: [48.35, 16.60, 48.10, 16.20] },
  { id: 'at-west', bbox: [47.80, 13.20, 46.40, 9.50] },
  { id: 'at-east', bbox: [49.05, 17.20, 46.60, 13.10] },
  { id: 'de-north', bbox: [55.10, 14.50, 52.20, 5.80] },
  { id: 'de-mid', bbox: [52.30, 14.50, 50.00, 5.80] },
  { id: 'de-south', bbox: [50.10, 13.90, 47.25, 7.50] },
  { id: 'de-berlin', bbox: [52.70, 13.80, 52.30, 13.05] },
  { id: 'ch', bbox: [47.85, 10.55, 45.80, 5.90] },
  { id: 'cz-sk', bbox: [51.10, 22.60, 47.70, 12.10] },
  { id: 'hu-ro', bbox: [48.30, 29.80, 43.60, 16.10] },
  { id: 'it-north', bbox: [47.20, 14.00, 43.50, 6.40] },
  { id: 'it-south', bbox: [43.60, 18.80, 36.40, 8.00] },
  { id: 'fr-north', bbox: [51.60, 8.40, 46.50, -5.20] },
  { id: 'fr-south', bbox: [46.60, 8.40, 42.20, -2.20] },
  { id: 'benelux', bbox: [53.70, 7.30, 49.40, 2.50] },
  { id: 'iberia-north', bbox: [44.00, 3.60, 40.00, -9.80] },
  { id: 'iberia-south', bbox: [40.10, 1.60, 35.80, -9.60] },
  { id: 'nordics-south', bbox: [62.00, 31.60, 54.60, 4.20] },
  { id: 'nordics-north', bbox: [71.20, 31.60, 62.10, 4.20] },
  { id: 'uk-south', bbox: [54.00, 2.00, 49.80, -6.50] },
  { id: 'uk-north-ie', bbox: [59.40, 0.00, 53.90, -10.80] },
  { id: 'balkans', bbox: [47.00, 23.00, 39.00, 13.30] },
  { id: 'greece-tr', bbox: [42.00, 30.00, 34.80, 19.00] },
  { id: 'pl', bbox: [55.00, 24.20, 49.00, 14.10] },
  { id: 'baltics', bbox: [59.80, 28.40, 53.80, 20.80] },
];

/**
 * US anchor boxes. Europe is this build's primary theatre, but the US layers
 * stay populated — these keep transatlantic coverage without letting a single
 * global query crowd Europe out.
 */
const WINDY_US_BOXES = [
  { id: 'us-west', bbox: [49.0, -104.0, 31.0, -125.0] },
  { id: 'us-east', bbox: [47.5, -67.0, 25.0, -90.0] },
];

/**
 * Windy catalog cache TTL, deliberately much longer than CCTV_SOURCE_CACHE_MS.
 *
 * Budget arithmetic: the free tier allows 1000 requests/day and this loader
 * issues one request PER BOX. At the 15-minute catalog TTL that would be
 * 96 refreshes x 15 boxes = 1440 requests/day — over the limit. Camera
 * POSITIONS are near-static, and the live frames are served from Windy's image
 * CDN through /api/cctv/frame (not the metered API), so the catalog can age for
 * hours with no visible staleness. 6h x 15 boxes = 60 requests/day.
 */
const WINDY_CACHE_MS = 6 * 60 * 60 * 1000;
let _windyCache = [];
let _windyCacheAt = 0;

/**
 * Load public webcams from the Windy Webcams API.
 *
 * This is the only source found that actually covers continental European
 * cities (Vienna, Germany, and the rest): the national road operators either
 * publish no open camera API at all (ASFINAG) or ship an endpoint that returns
 * an empty list for every road (the German Autobahn webcam service, verified
 * empty across A1-A61). Windy aggregates ~71k public cameras, ~7.7k of them in
 * the European `traffic` category.
 *
 * Frames are served through this app's existing `/api/cctv/frame/:id` proxy
 * (`snapshotUrl`), so the key never reaches the browser and Windy sees one
 * request per camera refresh rather than one per viewer.
 *
 * @returns {Promise<Array<object>>} Normalized camera sources (never throws).
 */
async function loadWindyWebcamSources() {
  const apiKey = String(process.env.WINDY_WEBCAMS_KEY || '').trim();
  if (!apiKey) return [];

  // Serve the long-lived catalog cache before spending any request budget.
  if (_windyCache.length && Date.now() - _windyCacheAt <= WINDY_CACHE_MS) {
    return _windyCache;
  }
  ...
  // Europe first and with the larger share of the budget — this build's primary
  // theatre — then the US boxes so those layers stay populated too.
  const includeUs = String(process.env.CCTV_WINDY_INCLUDE_US || '1').trim() !== '0';
  const boxes = includeUs
    ? [...WINDY_EUROPE_BOXES, ...WINDY_US_BOXES]
    : WINDY_EUROPE_BOXES;
  const perBox = Math.max(4, Math.ceil(maxCount / boxes.length));

  const fetchBox = async (box) => {
    const params = new URLSearchParams({
      bbox: box.bbox.join(','),
      limit: String(Math.min(50, perBox)),
      include: 'images,location',
      // No `categories` filter. Restricting to traffic,city cut Vienna from 135
      // cameras to 5 and Austria from 4782 to 38 — for a global-observation
      // console every public camera is in scope, not just road cameras.
      // Busiest cameras first: view count is the only quality signal the API
      // exposes, and an unranked slice is mostly dead/parked cameras.
      sortBy: 'popularity',
    });
    const resp = await fetch(`${WINDY_WEBCAMS_URL}?${params}`, {
      headers: { Accept: 'application/json', 'x-windy-api-key': apiKey },
      signal: AbortSignal.timeout(CCTV_SOURCE_FETCH_TIMEOUT_MS),
    });
    if (!resp.ok) {
      console.warn(`[CCTV] Windy box ${box.id} failed: HTTP ${resp.status}`);
      return [];
    }
    const payload = await resp.json();
  ...
  const settled = await Promise.allSettled(boxes.map(fetchBox));
  const cameras = [];
  const seen = new Set();
  ...
  for (const result of settled) {
    ...
    for (const cam of result.value) {
      // Only cameras Windy itself reports as live get into the registry; an
      // inactive camera would otherwise occupy a slot and render a stale frame.
      if (String(cam?.status || '').toLowerCase() !== 'active') continue;
      const id = toFiniteNumber(cam?.webcamId);
      if (!Number.isFinite(id) || seen.has(id)) continue;
      const loc = cam?.location || {};
      const lat = toFiniteNumber(loc.latitude);
      const lon = toFiniteNumber(loc.longitude);
      if (!Number.isFinite(lat) || !Number.isFinite(lon)) continue;
      // `full` is the largest still the free tier serves; `preview` is 352x224
      // and reads visibly soft once draped into the 3D scene.
      const frameUrl = `https://imgproxy.windy.com/_/full/plain/current/${id}/original.jpg?v=2`;
      seen.add(id);

      const cameraId = `windy-${id}`;
      cameras.push({
        id: cameraId,
        name: String(cam?.title || `Webcam ${id}`).trim(),
        city: String(loc.city || loc.region || loc.country || 'Unknown'),
        cityId: String(loc.country_code || 'xx').toLowerCase(),
        provider: 'Windy Webcams',
        lat,
        lon,
        // Windy exposes no bearing. Same treatment as headingless Austin/TfL
        // cameras: a stable id-derived prior, flagged low so the calibration
        // gizmo (and the app's own confidence rules) know not to trust it.
        headingDeg: fallbackHeadingFromId(cameraId),
        headingConfidence: 'low',
        pitchDeg: -12,
        fovDeg: 52,
  ...
  const result = cameras.slice(0, maxCount);
  // Only replace a warm catalog with a non-empty refresh: a transient upstream
  // failure must not blank Europe's cameras for the next six hours.
  if (result.length || !_windyCache.length) {
    _windyCache = result;
    _windyCacheAt = Date.now();
  }
  return _windyCache;
}

/** @const {string} Fintraffic Digitraffic weather-camera station list. */
const DIGITRAFFIC_STATIONS_URL = 'https://tie.digitraffic.fi/api/weathercam/v1/stations';
/** @const {number} Default cap on Finnish cameras pulled into the registry. */
const DEFAULT_DIGITRAFFIC_MAX_SOURCES = 120;

/**
 * Load Finnish road cameras from Fintraffic's Digitraffic open API.
 *
 * Keyless and unrestricted, ~810 stations. The station list carries each
 * camera's presets (fixed viewing directions) with a direct image URL, so the
 * frames need no separate lookup. Digitraffic REQUIRES gzip and a identifying
 * `Digitraffic-User` header — a plain request is rejected with HTTP 406.
 *
 * @returns {Promise<Array<object>>} Normalized camera sources (never throws).
 */
async function loadDigitrafficSourcesFromOpenData() {
  if (String(process.env.CCTV_DIGITRAFFIC_ENABLED || '1').trim() === '0') return [];
  try {
    const resp = await fetch(DIGITRAFFIC_STATIONS_URL, {
      headers: {
        Accept: 'application/json',
        'Accept-Encoding': 'gzip',
        'Digitraffic-User': 'JP-GOD-EYE/1.0',
      },
      signal: AbortSignal.timeout(CCTV_SOURCE_FETCH_TIMEOUT_MS),
    });
    if (!resp.ok) {
      console.warn('[CCTV] Digitraffic station list failed:', resp.status);
      return [];
    }
    const payload = await resp.json();
    const features = Array.isArray(payload?.features) ? payload.features : [];

    const maxRaw = Number(process.env.CCTV_DIGITRAFFIC_MAX_SOURCES || DEFAULT_DIGITRAFFIC_MAX_SOURCES);
    const maxCount = Number.isFinite(maxRaw)
      ? Math.max(0, Math.min(810, Math.floor(maxRaw)))
      : DEFAULT_DIGITRAFFIC_MAX_SOURCES;
    if (maxCount === 0) return [];

    const cameras = [];
    for (const feature of features) {
      if (cameras.length >= maxCount) break;
      const props = feature?.properties || {};
      const coords = feature?.geometry?.coordinates;
      const lon = toFiniteNumber(Array.isArray(coords) ? coords[0] : undefined);
      const lat = toFiniteNumber(Array.isArray(coords) ? coords[1] : undefined);
      if (!Number.isFinite(lat) || !Number.isFinite(lon)) continue;

      // One source per STATION (its first collecting preset), not per preset: a
      // station's presets are the same mast pointing different ways, and three
      // stacked markers on one mast is noise on the globe.
      //
      // The station LIST carries only `{id, inCollection}` per preset — the
      // `imageUrl` field exists solely on the per-station detail endpoint.
      // Fetching 810 detail documents to learn a deterministic URL would be
      // absurd, so the frame URL is derived from the preset id, the documented
      // and verified pattern (`C0150301` -> weathercam.digitraffic.fi/C0150301.jpg).
      const preset = (Array.isArray(props.presets) ? props.presets : [])
        .find((item) => item?.inCollection !== false && /^[A-Za-z0-9_-]{4,32}$/.test(String(item?.id || '')));
      if (!preset) continue;
      const presetId = String(preset.id);
      const imageUrl = `https://weathercam.digitraffic.fi/${presetId}.jpg`;

      const stationId = String(feature?.id || props.id || '').trim();
      if (!stationId) continue;
      const cameraId = `digitraffic-${stationId}`;
      cameras.push({
        id: cameraId,
        name: String(props.name || `Kamera ${stationId}`),
        ...
        url: imageUrl,
        snapshotUrl: imageUrl,
        sourceKind: 'digitraffic-open-data',
        license: 'Fintraffic / Digitraffic (CC BY 4.0)',
      });
    }
    return cameras;
  } catch (error) {
    console.warn('[CCTV] Digitraffic load failed:', error?.message || error);
    return [];
  }
}

// ── Europe-first situational layers (all keyless) ───────────────────────────
//
// Each endpoint normalizes one public feed into a GeoJSON FeatureCollection so
// the existing `createLocalGeoJsonLayer` renderer can draw it unchanged — no
// new layer engine, no new client abstraction. Every one is cached in memory,
// serves its last good payload during an upstream blip, and never fabricates a
// feature: an unreachable source yields an EMPTY collection plus an `error`,
// which the layer surfaces as an unavailable state.
const SITUATION_CACHE_MS = 5 * 60 * 1000;
const _situationCache = new Map();

/**
 * Fetch + normalize one situational feed behind a shared cache.
 * @param {string} key Cache key.
 * @param {() => Promise<object>} build Produces the FeatureCollection.
 * @returns {Promise<object>} GeoJSON FeatureCollection (possibly empty).
 */
async function serveSituationFeed(key, build) {
  const now = Date.now();
  const hit = _situationCache.get(key);
  if (hit && now - hit.at <= SITUATION_CACHE_MS) return hit.value;
  try {
    const value = await build();
    _situationCache.set(key, { at: now, value });
    return value;
  } catch (error) {
    // Serve the last good payload rather than blanking a layer on one blip.
    if (hit) return { ...hit.value, stale: true };
    return { type: 'FeatureCollection', features: [], error: String(error?.message || error) };
  }
}

const situationJson = async (url, headers = {}) => {
  const resp = await fetch(url, {
    headers: { Accept: 'application/json', 'User-Agent': 'JP-GOD-EYE/1.0', ...headers },
    signal: AbortSignal.timeout(20000),
  });
...
/** EMSC European-Mediterranean seismic events — finer than USGS over Europe. */
async function buildEmscFeed() {
  const d = await situationJson(
    'https://www.seismicportal.eu/fdsnws/event/1/query?format=json&limit=400&minlat=30&maxlat=75&minlon=-30&maxlon=50&minmag=1'
  );
  const features = [];
  for (const f of d?.features || []) {
    ...
    features.push(feature(c[0], c[1], {
      name: `M${p.mag} ${p.flynn_region || ''}`.trim(),
      magnitude: p.mag,
      depthKm: p.depth,
      time: p.time,
    }));
...
/** GDACS (EU Joint Research Centre) global disaster alerts. */
async function buildGdacsFeed() {
  const d = await situationJson(
    'https://www.gdacs.org/gdacsapi/api/events/geteventlist/SEARCH?fromDate=&toDate=&alertlevel=Green;Orange;Red&eventlist=EQ;TC;FL;VO;WF;DR'
  );
  ...
  for (const f of d?.features || []) {
    ...
    if (!Array.isArray(c) || !Number.isFinite(c[0]) || !Number.isFinite(c[1])) continue;
    features.push(feature(c[0], c[1], {
      name: `${KIND[p.eventtype] || p.eventtype || 'Event'} · ${p.name || p.htmldescription || ''}`.slice(0, 90),
...
/** German Autobahn roadworks + closures (Autobahn GmbH open API). */
async function buildAutobahnFeed() {
  const roads = (await situationJson('https://verkehr.autobahn.de/o/autobahn/'))?.roads || [];
  // Bounded fan-out: the full list is ~130 roads and each is a separate
  // request. The busiest corridors carry most of the disruption and keep this
  // to one bounded burst rather than a 260-request storm every refresh.
  const wanted = roads.slice(0, 28);
  const features = [];
  const grab = async (road, service, kind) => {
    try {
      const d = await situationJson(`https://verkehr.autobahn.de/o/autobahn/${encodeURIComponent(road)}/services/${service}`);
      for (const item of d?.[service] || []) {
        const lon = Number(item?.coordinate?.long);
        const lat = Number(item?.coordinate?.lat);
        if (!Number.isFinite(lon) || !Number.isFinite(lat)) continue;
        features.push(feature(lon, lat, {
          name: `${road} · ${kind}: ${String(item?.title || '').slice(0, 70)}`,
          road,
          kind,
          subtitle: item?.subtitle || '',
        }));
      }
    } catch { /* one road failing must not empty the layer */ }
  };
  await Promise.allSettled([
    ...wanted.map((r) => grab(r, 'roadworks', 'Baustelle')),
    ...wanted.map((r) => grab(r, 'closure', 'Sperrung')),
  ]);
  return { type: 'FeatureCollection', features };
}

/** NOAA SWPC OVATION aurora forecast, thinned to a drawable set. */
async function buildAuroraFeed() {
  const d = await situationJson('https://services.swpc.noaa.gov/json/ovation_aurora_latest.json');
  const features = [];
  for (const row of d?.coordinates || []) {
    const [lon, lat, prob] = row;
    // The raw grid is 65k points at 1-degree spacing. Only meaningful
    // probabilities are worth drawing, and the sub-sample keeps the entity
    // count in the low thousands instead of melting the globe.
    if (!Number.isFinite(prob) || prob < 12) continue;
    if (Math.abs(lat) < 40) continue;
    if (lon % 3 !== 0 || lat % 2 !== 0) continue;
    features.push(feature(lon > 180 ? lon - 360 : lon, lat, {
      name: `Aurora ${prob}%`,
      probability: prob,
    }));
  }
  return { type: 'FeatureCollection', features, observedAt: d?.['Observation Time'] || null };
}

/** Open-Meteo air quality sampled across a European grid. */
async function buildAirQualityFeed() {
  // One batched request: Open-Meteo accepts comma-separated coordinate lists,
  // so a 5-degree European grid costs a single call rather than ~90.
  const lats = [];
  const lons = [];
  for (let lat = 36; lat <= 68; lat += 4) {
    for (let lon = -8; lon <= 30; lon += 5) { lats.push(lat); lons.push(lon); }
  }
  const d = await situationJson(
    `https://air-quality-api.open-meteo.com/v1/air-quality?latitude=${lats.join(',')}&longitude=${lons.join(',')}&current=european_aqi,pm10,pm2_5`
  );
...
const SITUATION_FEEDS = {
  emsc: buildEmscFeed,
  gdacs: buildGdacsFeed,
  autobahn: buildAutobahnFeed,
  aurora: buildAuroraFeed,
  'air-quality': buildAirQualityFeed,
};
...
async function refreshCctvSources() {
  ...
  let fromDigitraffic = [];
  if (needsLiveSources) {
    const [austinResult, caltransResult, tflResult, windyResult, digitrafficResult] =
      await Promise.allSettled([
        loadAustinSourcesFromOpenData(),
        loadCaltransSourcesFromOpenData(),
        tflEnabled ? loadTflSourcesFromOpenData() : Promise.resolve([]),
        loadWindyWebcamSources(),
    ...
    fromDigitraffic = digitrafficResult.status === 'fulfilled' ? digitrafficResult.value : [];
  }
  // European packs lead the merge. The catalog is capped further down by
  // CCTV_MAX_SOURCES and that cap keeps the FIRST N entries, so ordering here is
  // what decides who survives a cap — Europe is this build's primary theatre,
  // and the US packs follow rather than being displaced.
  const merged = [
    ...fromWindy,
    ...fromDigitraffic,
    ...fromTfl,
  ...
  // Ceiling raised from 1200 to 2000: the European packs alone now supply
...
export async function fetchCctvImageFromUpstream(url, {
  ...
  try {
    const upstream = await fetchImpl(url, {
      headers: { 'User-Agent': 'jp-god-eye-cctv-proxy/1.0' },
      signal: controller.signal,
    });
...
function cctvProxy() {
  ...
  /**
   * Fetch a street-level fallback frame from Mapillary for a camera whose own
   * upstream is down.
   *
   * Replaces the metered Google Street View Static fallback. Mapillary is
   * community-captured imagery under CC BY-SA 4.0, reachable with a free
   * developer token and no billing account. Entirely OPTIONAL: with no
   * MAPILLARY_TOKEN this returns null and the caller falls through to the
   * existing honest "UPSTREAM UNAVAILABLE" placeholder — it never invents a
   * frame.
   *
   * Picks the capture whose compass angle is closest to the camera's real
   * bearing, so the substitute frame at least looks the way the camera does.
   * @returns {Promise<{ok: true, body: Buffer, contentType: string, attribution: string}|null>}
   */
  const streetLevelFallback = async ({ lat, lon, heading }) => {
    const token = process.env.MAPILLARY_TOKEN;
    if (!token || !Number.isFinite(lat) || !Number.isFinite(lon)) return null;
    try {
      // Mapillary caps a search bbox at 0.01 square degrees; ~±75 m keeps the
      // window far inside that and keeps the result near the actual camera.
      const half = 0.0007;
      const bbox = [lon - half, lat - half, lon + half, lat + half]
        .map((value) => value.toFixed(6))
        .join(',');
      const search = new URL('https://graph.mapillary.com/images');
      search.searchParams.set('bbox', bbox);
      search.searchParams.set('fields', 'id,compass_angle,thumb_1024_url,captured_at');
      search.searchParams.set('limit', '12');

      // Token travels in the Authorization header, not the query string, as
      // Mapillary's own documentation requires for access tokens.
      const searchResponse = await fetch(search.toString(), {
        headers: {
          Authorization: `OAuth ${token}`,
          'User-Agent': 'jp-god-eye-cctv-proxy/1.0',
        },
      ...
      const target = Number.isFinite(heading) ? ((heading % 360) + 360) % 360 : null;
      const bearingDelta = (angle) => {
        if (target === null || !Number.isFinite(angle)) return 360;
        const diff = Math.abs((((angle - target) % 360) + 540) % 360 - 180);
        return 180 - diff;
      };
      const best = candidates
        .filter((image) => typeof image?.thumb_1024_url === 'string')
        .sort((a, b) => (
          bearingDelta(a.compass_angle) - bearingDelta(b.compass_angle)
          || (b.captured_at || 0) - (a.captured_at || 0)
        ))[0];
      if (!best) return null;

      // The thumbnail lives on a Mapillary-signed CDN URL that the search
      // response just handed us — fetched as-is, never from client input.
      const thumbUrl = new URL(best.thumb_1024_url);
      if (thumbUrl.protocol !== 'https:') return null;
      const imageResponse = await fetch(thumbUrl.toString(), {
        headers: { 'User-Agent': 'jp-god-eye-cctv-proxy/1.0' },
        signal: AbortSignal.timeout(CCTV_FRAME_FETCH_TIMEOUT_MS),
      });
      ...
      server.middlewares.use('/api/cctv', async (req, res) => {
        try {
          ...
          if (url.pathname.startsWith('/media/')) {
            ...
            try {
              const upstreamHeaders = { 'User-Agent': 'jp-god-eye-cctv-proxy/1.0' };
          ...
          const streetLevel = await streetLevelFallback({ lat, lon, heading });
          if (streetLevel?.ok) {
            setHealth(cameraId, {
              status: 'degraded',
              sourceKind: 'mapillary',
              label: 'Mapillary',
              message: 'Fallback street-level frame',
            });
            res.writeHead(200, {
              'Content-Type': streetLevel.contentType,
              'Cache-Control': 'no-store',
              'X-CCTV-Source': 'mapillary',
              // CC BY-SA 4.0 requires attribution to travel with the image, so
              // it rides the response rather than living only in the docs.
              'X-CCTV-Attribution': streetLevel.attribution,
            });
            res.end(streetLevel.body);
            return;
          }
      ...
      server.middlewares.use('/api/adsblol/mil', async (req, res) => {
        try {
          ...
          const upstream = await fetch('https://api.adsb.lol/v2/mil', {
            headers: { 'User-Agent': 'jp-god-eye-adsblol-proxy/1.0' },
          });
          const body = await upstream.text();
          if (upstream.ok) {
            _cache = body;
...
function aisLiveProxy() {
  function install(middlewares) {
    // One route serves all five situational feeds; the name is validated
    // against the registry so the path can never reach an arbitrary upstream.
    // Naked-eye satellite passes.
    //
    // The app already predicts passes locally from CelesTrak TLEs via SGP4
    // (src/data/issPass.js), which needs no key and no network. What that
    // computation does NOT model is ILLUMINATION — whether the satellite is
    // sunlit while the observer is in darkness, which is the difference
    // between "above the horizon" and "actually visible". N2YO's visualpasses
    // answers exactly that, so this endpoint is a strict upgrade layered over
    // the local prediction rather than a replacement for it.
    //
    // Without N2YO_API_KEY it returns `configured: false` and the caller keeps
    // using the local geometric prediction.
    middlewares.use('/api/satellite-passes', async (req, res) => {
      ...
      try {
        ...
        // NORAD id, defaulting to the ISS. Digits only — this value is
        // interpolated into the upstream path.
        const rawId = String(url.searchParams.get('satid') || '25544').trim();
        const days = Math.max(1, Math.min(10, Number(url.searchParams.get('days')) || 5));
        if (!Number.isFinite(lat) || !Number.isFinite(lon) || Math.abs(lat) > 90 || Math.abs(lon) > 180) {
          return json(400, { error: 'Valid lat and lon are required' });
        }
        if (!/^\d{1,6}$/.test(rawId)) return json(400, { error: 'satid must be a NORAD number' });

        const apiKey = String(process.env.N2YO_API_KEY || '').trim();
        if (!apiKey) {
          return json(200, {
            configured: false,
            passes: [],
            note: 'N2YO_API_KEY not set; visible-pass filtering unavailable (local geometric prediction still applies)',
          });
        }

        // 0.5-degree cache cells: pass geometry barely changes across ~50 km,
        // and N2YO's free tier is 1000 requests/hour.
        const cacheKey = `${rawId}|${lat.toFixed(1)}|${lon.toFixed(1)}|${days}`;
        const payload = await serveSituationFeed(`n2yo:${cacheKey}`, async () => {
          const upstream = `https://api.n2yo.com/rest/v1/satellite/visualpasses/${rawId}/${lat}/${lon}/0/${days}/60&apiKey=${encodeURIComponent(apiKey)}`;
          const resp = await fetch(upstream, { signal: AbortSignal.timeout(20000) });
          if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
          const data = await resp.json();
          const passes = (Array.isArray(data?.passes) ? data.passes : []).map((pass) => ({
            startUtc: Number(pass.startUTC) * 1000,
            endUtc: Number(pass.endUTC) * 1000,
            maxElevationDeg: Number(pass.maxEl),
            startAzDeg: Number(pass.startAz),
            endAzDeg: Number(pass.endAz),
            durationSec: Number(pass.duration),
            magnitude: Number(pass.mag),
          })).filter((pass) => Number.isFinite(pass.startUtc) && Number.isFinite(pass.maxElevationDeg));
          return {
            configured: true,
            satelliteName: data?.info?.satname || null,
            passes,
          };
        });
        return json(200, payload);
      } catch (error) {
        return json(502, { error: error?.message || 'Satellite pass lookup failed' });
      }
    });

    // ── Setup wizard ────────────────────────────────────────────────────
    //
    // Writes API keys into `.env` from a local web form. This is a CONFIG-WRITE
    // endpoint, so it is fenced on three sides:
    //
    //   1. LOOPBACK ONLY. A request whose socket is not 127.0.0.1 / ::1 is
    //      refused outright. Without this, anyone who could reach a
    //      LAN-exposed dev server could rewrite the operator's credentials.
    //   2. ALLOWLIST. Only names in SETUP_KEYS are accepted; everything else
    //      is rejected before it can reach the file.
    //   3. VALUE VALIDATION. Shape-checked and newline-free, so one field can
    //      never become two variables.
    //
    // Values are never read back out — the status route reports only WHICH
    // keys are set, never what they are.
    ...
    middlewares.use('/api/setup', async (req, res) => {
      const json = (status, body) => {
        res.writeHead(status, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' });
        res.end(JSON.stringify(body));
      };
      if (!isLoopbackRequest(req)) {
        return json(403, { error: 'Setup is only available from this machine' });
      }
      try {
        const url = new URL(req.url || '/', 'http://localhost');
        ...
        if (url.pathname === '/status' || url.pathname === '/') {
          ...
          const configured = {};
          for (const spec of SETUP_KEYS) {
            // Presence only. The value never leaves the server.
            const match = new RegExp(`^${spec.name}=(.+)$`, 'm').exec(current);
            configured[spec.name] = Boolean(match && match[1].trim());
          }
          return json(200, { keys: SETUP_KEYS, configured });
        }
        ...
        if (url.pathname === '/save') {
          ...
          for (const [name, value] of Object.entries(submitted)) {
            const check = validateSetupValue(name, value);
            if (!check.ok) { errors[name] = check.error; continue; }
            updates[name] = String(value).trim();
          }
          if (Object.keys(errors).length) return json(400, { errors });

          fs.writeFileSync(envPath, mergeEnv(readEnv(), updates), { mode: 0o600 });

          // OSIRIS is a separate Next.js process reading its own directory, so
          // the keys it consumes are mirrored there under the names it expects.
          const osirisDir = path.resolve(__dirname, 'osiris');
          if (fs.existsSync(osirisDir)) {
            const osirisPath = path.join(osirisDir, '.env.local');
            const mirrored = {};
            for (const [name, value] of Object.entries(updates)) {
              mirrored[name] = value;
              if (name === 'AISSTREAM_API_KEY') mirrored.AIS_API_KEY = value;
              if (name === 'FIRMS_MAP_KEY') mirrored.FIRMS_API_KEY = value;
            }
            const priorOsiris = fs.existsSync(osirisPath) ? fs.readFileSync(osirisPath, 'utf8') : '';
            fs.writeFileSync(osirisPath, mergeEnv(priorOsiris, mirrored), { mode: 0o600 });
          }

          return json(200, {
            saved: Object.keys(updates).filter((k) => updates[k] !== ''),
            cleared: Object.keys(updates).filter((k) => updates[k] === ''),
            // The dev server reads process.env at request time for most proxies,
            // but client-exposed values are baked at build time — say so rather
            // than implying everything is live immediately.
            restartRequired: true,
          });
        }

        return json(404, { error: 'Unknown setup route' });
      } catch (error) {
        return json(500, { error: error?.message || 'Setup failed' });
      }
    });

    middlewares.use('/api/situation', async (req, res) => {
      try {
        const url = new URL(req.url || '/', 'http://localhost');
        const name = url.pathname.replace(/^\//, '').trim();
        const build = Object.prototype.hasOwnProperty.call(SITUATION_FEEDS, name)
          ? SITUATION_FEEDS[name]
          : null;
        if (!build) {
          res.writeHead(404, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ error: 'Unknown situational feed' }));
          return;
        }
        const payload = await serveSituationFeed(name, build);
        res.writeHead(200, {
        ...
        res.writeHead(502, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: error?.message || 'Situational feed failed' }));
      }
    });
...
/* ------------------------------------------------------------------------ */
/* KEYLESS GEO PROXY — the free replacement for Google Geocoding + Places.    */
/* ------------------------------------------------------------------------ */

/** @const {number} Nominatim's published policy is max 1 request/second. */
const NOMINATIM_MIN_INTERVAL_MS = 1100;

/** Contact string Nominatim's policy requires on every request. */
const OSM_UA = 'JpGodEye/1.0 (+https://github.com/JakobPapaj/jp-god-eye)';
const OSM_HEADERS = { 'User-Agent': OSM_UA, Referer: 'https://github.com/JakobPapaj/jp-god-eye' };

// Geocodes are stable facts about the world, so they are cached hard: a place
// does not move between restarts, and every cache hit is one request the shared
// public Nominatim instance does not have to serve.
const GEOCODE_CACHE_MS = 7 * 24 * 60 * 60_000;
const GEOCODE_MAX_CACHE = 600;
const REVERSE_CACHE_MS = 24 * 60 * 60_000;
const REVERSE_MAX_CACHE = 900;
const PLACES_CACHE_MS = 60 * 60_000;
const PLACES_MAX_CACHE = 400;
const GEO_MAX_RESPONSE_BYTES = 1024 * 1024;

const _geocodeCache = new Map();
const _reverseCache = new Map();
const _placesCache = new Map();
const _geoInFlight = new Map();
const _geoRateLimiter = makeRateLimiter({ windowMs: 60_000, max: 40, globalMax: 120 });

/**
 * Serializes every Nominatim request in this process behind one 1 req/s gate.
 *
 * Nominatim's usage policy is a per-CLIENT limit, not a per-endpoint one, so
 * the geocode, reverse-geocode and cockpit regional-brief paths must share a
 * single queue. Giving each its own timer would let three compliant-looking
 * endpoints add up to three requests per second.
 * @param {() => Promise<*>} task
 * @returns {Promise<*>}
 */
function queueNominatim(task) {
  ...
  // Keep the chain alive after a rejection; a failed geocode must not wedge
  // every later Nominatim request behind a permanently rejected promise.
  _nominatimQueue = run.catch(() => null);
  return run;
}

/** LRU-ish trim: Map preserves insertion order, so the first key is the oldest. */
function trimGeoCache(cache, max) {
...
function readGeoCache(cache, key, maxAgeMs) {
...
function writeGeoCache(cache, key, value, max) {
  cache.set(key, { value, cachedAt: Date.now() });
  trimGeoCache(cache, max);
}

/**
 * Coalesces concurrent identical lookups onto one upstream request. Three
 * viewport samples rounding to the same cell must cost one Nominatim slot,
 * not three.
 */
function dedupeGeoRequest(key, run) {
  const pending = _geoInFlight.get(key);
  if (pending) return pending;
  const promise = run().finally(() => _geoInFlight.delete(key));
  _geoInFlight.set(key, promise);
  return promise;
}

function sendGeoJson(res, status, payload, { cacheSeconds = 0 } = {}) {
...
function normalizeNominatimViewport(boundingbox) {
...
/**
 * Normalizes one Nominatim search row into the shape the client geocode
 * consumers expect. `name` is Nominatim's own canonical feature name, which is
 * strictly better than the heuristic that used to pick a "primary name" out of
 * Google's address components.
 */
function normalizeGeocodeRow(row) {
  const latitude = Number(row?.lat);
  const longitude = Number(row?.lon);
  if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) return null;
  const displayName = String(row?.display_name || '').trim();
  const category = String(row?.category || row?.class || '').trim() || null;
  const type = String(row?.type || '').trim() || null;
  const addressType = String(row?.addresstype || '').trim() || null;
  return {
    name: String(row?.name || '').trim() || (displayName.split(',')[0] || '').trim() || null,
    ...
    type,
    addressType,
    // Nominatim's relevance score. Preserved so the client can prefer a
    // well-known feature over an obscure same-named one.
    importance: Number.isFinite(Number(row?.importance)) ? Number(row.importance) : null,
    types: geocodeTypeTokens({ addressType, category, type }),
    viewport: normalizeNominatimViewport(row?.boundingbox),
  };
}

/**
 * Translate an OSM feature classification into the place-type vocabulary the
 * camera-framing policy already speaks (`geocodeNavigationMode` in
 * locations.js).
 *
 * Framing behaviour — whether "Texas" frames a region, "Zilker Park" frames an
 * area, and "Sixth Street" frames a corridor — is a tuned product decision that
 * predates this migration. Emitting the same tokens from OSM data keeps that
 * decision intact instead of re-deriving it against a second vocabulary. The
 * raw `osm:` tokens ride along so nothing downstream loses the real source
 * classification.
 * @param {{addressType: string|null, category: string|null, type: string|null}} input
 * @returns {string[]}
 */
export function geocodeTypeTokens({ addressType, category, type }) {
  const tokens = new Set();
  if (category) tokens.add(`osm:${category}`);
  if (type) tokens.add(`osm:${type}`);

  // `addresstype` is Nominatim's own answer to "what KIND of place is this",
  // and is far more dependable than guessing from class/type pairs.
  const byAddressType = {
    country: ['country'],
    ...
    village: ['locality'],
    hamlet: ['locality'],
    borough: ['sublocality', 'neighborhood'],
  ...
  for (const token of byAddressType[addressType] || []) tokens.add(token);

  // Feature classes that change framing but are not administrative places.
  const byClassType = {
    'highway:*': ['route'],
    'leisure:park': ['park'],
    'leisure:garden': ['park'],
    ...
    'tourism:museum': ['tourist_attraction'],
    'historic:*': ['tourist_attraction'],
  };
  for (const token of byClassType[`${category}:${type}`] || []) tokens.add(token);
  // Wildcard families: every highway is a route, every natural feature reads as
  // one, regardless of the specific subtype.
  for (const token of byClassType[`${category}:*`] || []) tokens.add(token);

  return [...tokens].slice(0, 10);
}

/**
 * Forward geocode through Nominatim (primary).
 * @returns {Promise<Array<object>>}
 */
async function nominatimSearch({ query, limit, viewbox, bounded }) {
  const params = new URLSearchParams({
    q: query,
    format: 'jsonv2',
    limit: String(limit),
  ...
  const payload = await queueNominatim(() => fetchRegionalJson(
    `https://nominatim.openstreetmap.org/search?${params}`,
    { headers: OSM_HEADERS, maxBytes: GEO_MAX_RESPONSE_BYTES },
  ));
...
/**
 * Forward geocode through Photon (secondary).
 *
 * Photon is keyless and has no per-second gate, but its ranking is measurably
 * weaker than Nominatim's — a live check for "Eiffel Tower" returned a peak in
 * Alberta as the top hit. It therefore runs ONLY when Nominatim failed or
 * returned nothing, and its rows are marked with their real source so a caller
 * can tell the two apart.
 * @returns {Promise<Array<object>>}
 */
async function photonSearch({ query, limit, lat, lon }) {
  const params = new URLSearchParams({ q: query, limit: String(limit) });
  if (Number.isFinite(lat) && Number.isFinite(lon)) {
    params.set('lat', lat.toFixed(5));
    params.set('lon', lon.toFixed(5));
  }
  const payload = await fetchRegionalJson(`https://photon.komoot.io/api/?${params}`, {
    headers: { 'User-Agent': OSM_UA },
    maxBytes: GEO_MAX_RESPONSE_BYTES,
  });
  const features = Array.isArray(payload?.features) ? payload.features : [];
  return features.map((feature) => {
    ...
    if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) return null;
    const props = feature.properties || {};
    const category = String(props.osm_key || '').trim() || null;
    const type = String(props.osm_value || '').trim() || null;
    ...
    const extent = Array.isArray(props.extent) && props.extent.length === 4
      ? props.extent.map(Number)
      : null;
    return {
      name: String(props.name || '').trim() || null,
      displayName: displayName || null,
      latitude,
      longitude,
      category,
      type,
      importance: null,
      types: [category, type].filter(Boolean),
      viewport: extent && extent.every(Number.isFinite)
        ? {
          low: { latitude: extent[3], longitude: extent[0] },
          high: { latitude: extent[1], longitude: extent[2] },
        }
        : null,
    };
  }).filter(Boolean);
...
/** Normalize a Nominatim reverse response into the cockpit/voice place shape. */
function normalizeReversePlace(payload) {
  ...
  const formattedAddress = String(payload.display_name || '').trim() || null;
  const road = pick('road', 'pedestrian', 'footway', 'residential');
  const category = String(payload.category || payload.class || '').trim();
  const type = String(payload.type || '').trim();
  return {
    formattedAddress,
    name: String(payload.name || '').trim() || null,
    locality: pick('city', 'town', 'village', 'municipality', 'suburb', 'county'),
    region: pick('state', 'region', 'province', 'state_district'),
    country: pick('country'),
    types: [category, type].filter(Boolean),
    // Nominatim answers with ONE feature, where Google returned a ranked list.
    // Report exactly what came back rather than padding the array to look like
    // the old response — downstream summarizers count these labels.
    labels: formattedAddress ? [formattedAddress] : [],
    streetLabels: road ? [road] : [],
  };
}

/**
 * OSM tag families that carry a name worth reading back as scene context, and
 * the Google-style type tokens each maps to.
 *
 * Deliberately narrow: Overpass will happily return every bench and bin inside
 * the radius, which would bury the actual landmarks. `placeContextPriority()`
 * ranks the emitted tokens, so the mapping targets tokens it already scores.
 */
const OSM_PLACE_RULES = [
  { key: 'aeroway', values: ['aerodrome', 'terminal'], types: ['airport', 'point_of_interest'] },
  // `historic` and `military` are VALUE-FILTERED rather than matched on the
  // bare key. Unfiltered, `historic` alone pulls in every plaque and statue in
  // a city block — measured as both the slowest statement in the query and the
  // one that buried actual landmarks under street furniture.
  { key: 'historic', values: ['monument', 'memorial', 'castle', 'fort', 'ruins', 'archaeological_site', 'city_gate', 'aircraft', 'ship'], types: ['historical_landmark', 'monument', 'point_of_interest'] },
  { key: 'tourism', values: ['attraction', 'museum', 'viewpoint', 'theme_park', 'zoo', 'aquarium', 'gallery'], types: ['tourist_attraction', 'museum', 'point_of_interest'] },
  { key: 'leisure', values: ['park', 'stadium', 'garden', 'nature_reserve', 'marina', 'sports_centre'], types: ['park', 'point_of_interest'] },
  { key: 'amenity', values: ['place_of_worship', 'university', 'college', 'hospital', 'theatre', 'arts_centre', 'townhall', 'courthouse', 'library', 'police', 'fire_station', 'embassy', 'ferry_terminal', 'bus_station', 'prison'], types: ['point_of_interest'] },
  { key: 'man_made', values: ['tower', 'lighthouse', 'obelisk', 'bridge', 'water_tower', 'observatory'], types: ['monument', 'point_of_interest'] },
  { key: 'railway', values: ['station'], types: ['train_station', 'point_of_interest'] },
  { key: 'natural', values: ['peak', 'volcano', 'bay', 'beach', 'glacier'], types: ['natural_feature'] },
  { key: 'place', values: ['city', 'town', 'village', 'suburb', 'neighbourhood', 'quarter', 'island', 'borough'], types: ['locality'] },
  { key: 'military', values: ['airfield', 'base', 'barracks', 'bunker', 'naval_base', 'training_area'], types: ['point_of_interest'] },
];

/** Overpass QL regex-escape for a fixed tag value list. */
function osmValueRegex(values) {
  return `^(${values.join('|')})$`;
}

/**
 * Build the Overpass QL for named context features around a point.
 *
 * `nwr` collapses the node/way/relation triple into one statement per tag
 * family, which keeps the query at ten statements instead of thirty — the
 * difference between a query the public mirrors answer and one they time out.
 * Every interpolated value is a server-validated number or a constant from
 * OSM_PLACE_RULES, so nothing user-supplied reaches the QL.
 */
function buildNearbyPlacesOverpassQL(latitude, longitude, radiusM, limit) {
  const center = `${latitude.toFixed(5)},${longitude.toFixed(5)}`;
  const radius = Math.round(radiusM);
  // Tag filter FIRST, `["name"]` second. Overpass resolves the leading filter
  // against its tag index and the rest by scan, so `["name"]["tourism"]` asks
  // it to walk every named object in the radius before narrowing. Measured
  // against the live public mirror, flipping the order took a four-family
  // query from a 504 gateway timeout to ~2 s.
  const statements = OSM_PLACE_RULES.map(({ key, values }) => (
    `nwr["${key}"~"${osmValueRegex(values)}"]["name"](around:${radius},${center});`
  )).join('');
  // Overpass truncates at `out`, BEFORE this module's priority sort runs, so it
  // is asked for more rows than the caller wants — otherwise an arbitrary 24
  // survive and the landmark that should have ranked first can be cut.
  const overpassLimit = Math.min(150, Math.max(limit * 4, 60));
  return `[out:json][timeout:${OVERPASS_QL_TIMEOUT_S}];(${statements});out center ${overpassLimit};`;
}

/** @const {number} Server-side timeout Overpass itself enforces on our query. */
const OVERPASS_QL_TIMEOUT_S = 20;

/** Map an OSM element's tags onto the Google-style place type tokens. */
function osmPlaceTypes(tags) {
  const types = [];
  for (const rule of OSM_PLACE_RULES) {
    const value = tags?.[rule.key];
    if (!value) continue;
    if (!rule.values.includes(value)) continue;
    types.push(`${rule.key}:${value}`, ...rule.types);
  }
  return [...new Set(types)].slice(0, 8);
}

/** Normalize one Overpass element into the place shape the client consumes. */
function normalizeOverpassPlace(element, latitude, longitude) {
  const tags = element?.tags || {};
  const name = String(tags.name || '').trim();
  if (!name) return null;
  const placeLatitude = Number(element.lat ?? element.center?.lat);
  const placeLongitude = Number(element.lon ?? element.center?.lon);
  if (!Number.isFinite(placeLatitude) || !Number.isFinite(placeLongitude)) return null;
  const types = osmPlaceTypes(tags);
  ...
  const address = [tags['addr:street'], tags['addr:city']]
    ...
    .join(', ');
  return {
    id: element.type && element.id ? `osm:${element.type}/${element.id}` : null,
    name,
    address: address || null,
    latitude: placeLatitude,
    longitude: placeLongitude,
    distanceM: approximateDistanceM(latitude, longitude, placeLatitude, placeLongitude),
    // The highest-priority token doubles as the primary type, mirroring what
    // Google's `primaryType` meant to the consumers of this payload.
    primaryType: types.find((token) => !token.includes(':')) || types[0] || null,
    types,
    // Overpass `out center` returns a centroid, not a bounding box. Reporting
    // null is honest; the consumers that size a disc from a viewport fall back
    // to their own default rather than being handed an invented extent.
    viewport: null,
  };
}

/**
 * Run one Overpass query through the same mirror rotation, memory cache, disk
 * cache and serve-stale policy the public `/api/overpass` route uses.
 * @returns {Promise<{elements: Array<object>, stale: boolean}>}
 */
async function overpassQueryInternal(ql) {
  const cacheKey = ql.replace(/\s+/g, ' ').trim();
  const cached = _overpassCache.get(cacheKey);
  if (cached && Date.now() - cached.cachedAt <= PLACES_CACHE_MS) {
    return { elements: parseOverpassElements(cached.body), stale: false };
  }
  const inFlight = _overpassInFlight.get(cacheKey);
  const payload = inFlight
    ? await inFlight
    : await (() => {
      const request = fetchOverpassPayload(ql)
        .then((result) => {
          if (result.status < 500 && !result.rateLimited && !result.runtimeError) {
        ...
        .finally(() => _overpassInFlight.delete(cacheKey));
      _overpassInFlight.set(cacheKey, request);
      return request;
    })();

  if (payload.rateLimited || payload.runtimeError || payload.status >= 500) {
    // Every mirror is degraded. Last-good context beats an empty answer, and
    // the caller is told the rows are stale so it can say so.
    const stale = _overpassCache.get(cacheKey) || await readOverpassDisk(cacheKey, Infinity).catch(() => null);
    if (stale) return { elements: parseOverpassElements(stale.body), stale: true };
    throw new Error('Overpass unavailable on every mirror');
  }
  return { elements: parseOverpassElements(payload.body), stale: false };
}

function parseOverpassElements(body) {
  try {
    const parsed = JSON.parse(body);
...
/**
 * Vite plugin: keyless geocoding, reverse geocoding and place search.
 *
 * Replaces the Google Geocoding and Places endpoints this app used to broker.
 * Nothing here needs an API key, a billing account, or a provider agreement:
 *
 *   /api/geocode          → Nominatim (OSM), Photon as a real secondary
 *   /api/reverse-geocode  → Nominatim (OSM)
 *   /api/places/nearby    → Overpass (OSM), via the existing hardened pipeline
 *
 * All three are server-side so the OSM usage policy (contact header, 1 req/s,
 * caching) is enforced in ONE place rather than trusted to every browser tab.
 */
function keylessGeoProxy() {
  function install(middlewares) {
    middlewares.use('/api/geocode', async (req, res) => {
      if (req.method !== 'GET') {
        sendGeoJson(res, 405, { error: 'Method not allowed', results: [], source: null });
        return;
      }
      if (!_geoRateLimiter(clientKey(req))) {
        res.setHeader('Retry-After', '5');
        sendGeoJson(res, 429, { error: 'Rate limit exceeded', results: [], source: null });
        return;
      }
      ...
      if (!query) {
        sendGeoJson(res, 400, { error: 'Missing q', results: [], source: null });
        return;
      }
      const limit = Math.max(1, Math.min(10, Number(url.searchParams.get('limit')) || 5));
      // `viewbox` biases results toward what the operator is looking at, the
      // same job Google's `bounds` parameter did. Rebuilt from validated
      // numbers so a hostile querystring cannot be forwarded verbatim.
      const viewbox = parseGeocodeViewbox(url.searchParams.get('viewbox'));
      const bounded = url.searchParams.get('bounded') === '1';
      const lat = Number(url.searchParams.get('lat'));
      const lon = Number(url.searchParams.get('lon'));

      const cacheKey = `${query.toLowerCase()}|${limit}|${viewbox || ''}|${bounded ? 1 : 0}`;
      const cached = readGeoCache(_geocodeCache, cacheKey, GEOCODE_CACHE_MS);
      if (cached) {
        sendGeoJson(res, 200, cached, { cacheSeconds: 600 });
        return;
      }

      try {
        const payload = await dedupeGeoRequest(`geocode:${cacheKey}`, async () => {
          let results = [];
          let source = 'nominatim';
          let error = null;
          try {
            results = await nominatimSearch({ query, limit, viewbox, bounded });
          } catch (nominatimError) {
            error = nominatimError?.message || 'Nominatim unavailable';
            results = [];
          }
          if (!results.length) {
            try {
              const fallback = await photonSearch({ query, limit, lat, lon });
              if (fallback.length) {
                results = fallback;
                source = 'photon';
                error = null;
              }
            } catch { /* keep the Nominatim outcome; Photon is best-effort */ }
          }
          return { results, source: results.length ? source : null, error: results.length ? null : error };
        });
        // A definitive empty answer is cached too: re-asking the shared OSM
        // instance for a place that does not exist helps nobody.
        writeGeoCache(_geocodeCache, cacheKey, payload, GEOCODE_MAX_CACHE);
        sendGeoJson(res, 200, payload, { cacheSeconds: 600 });
      } catch (error) {
        sendGeoJson(res, 502, {
    ...
    middlewares.use('/api/reverse-geocode', async (req, res) => {
      if (req.method !== 'GET') {
        sendGeoJson(res, 405, { error: 'Method not allowed', place: null, source: null });
        return;
      }
      if (!_geoRateLimiter(clientKey(req))) {
        res.setHeader('Retry-After', '5');
        sendGeoJson(res, 429, { error: 'Rate limit exceeded', place: null, source: null });
        return;
      }

      const url = new URL(req.url || '/', 'http://localhost');
      // `requiredFiniteQueryNumber` returns null for an ABSENT or empty
      // parameter, which a bare `Number(...)` does not: `Number(null)` is 0, so
      // a request with no coordinates at all would otherwise pass validation and
      // be answered for 0°,0° — Null Island in the Gulf of Guinea — as though
      // that were what the caller asked about.
      const latitude = requiredFiniteQueryNumber(url.searchParams, 'lat');
      const longitude = requiredFiniteQueryNumber(url.searchParams, 'lon');
      if (
        latitude === null || longitude === null
        || Math.abs(latitude) > 90 || Math.abs(longitude) > 180
      ) {
        sendGeoJson(res, 400, { error: 'Invalid lat/lon', place: null, source: null });
        return;
      }
      const zoom = Math.max(3, Math.min(18, Number(url.searchParams.get('zoom')) || 14));

      // Round into ~10 m cells before caching. Camera samples drift constantly;
      // without this every frame-adjacent sample would be a fresh 1 req/s slot.
      const cacheKey = `${latitude.toFixed(4)},${longitude.toFixed(4)}|${zoom}`;
      const cached = readGeoCache(_reverseCache, cacheKey, REVERSE_CACHE_MS);
      if (cached) {
        sendGeoJson(res, 200, cached, { cacheSeconds: 600 });
        return;
      }
      ...
      try {
        const payload = await dedupeGeoRequest(`reverse:${cacheKey}`, async () => {
          const params = new URLSearchParams({
            format: 'jsonv2',
          ...
          const raw = await queueNominatim(() => fetchRegionalJson(
            `https://nominatim.openstreetmap.org/reverse?${params}`,
            { headers: OSM_HEADERS, maxBytes: GEO_MAX_RESPONSE_BYTES },
          ));
          return { place: normalizeReversePlace(raw), source: 'nominatim', error: null };
        });
        writeGeoCache(_reverseCache, cacheKey, payload, REVERSE_MAX_CACHE);
        sendGeoJson(res, 200, payload, { cacheSeconds: 600 });
      } catch (error) {
        sendGeoJson(res, 502, {
    ...
    middlewares.use('/api/places/nearby', async (req, res) => {
      if (req.method !== 'GET') {
        sendGeoJson(res, 405, { error: 'Method not allowed', places: [] });
        return;
      }
      if (!_geoRateLimiter(clientKey(req))) {
        res.setHeader('Retry-After', '5');
        sendGeoJson(res, 429, { error: 'Rate limit exceeded', places: [] });
        return;
      }

      const url = new URL(req.url || '/', 'http://localhost');
      // Absent coordinates must be a 400, not a silent query about 0°,0° —
      // see the note on the reverse-geocode route above.
      const latitude = requiredFiniteQueryNumber(url.searchParams, 'lat');
      const longitude = requiredFiniteQueryNumber(url.searchParams, 'lon');
      if (
        latitude === null || longitude === null
        ...
        sendGeoJson(res, 400, { error: 'Invalid lat/lon', places: [] });
        return;
      }
      // Capped at 20 km: an `around:` radius beyond that turns into planet-scale
      // work on a shared community mirror.
      const radiusM = Math.max(50, Math.min(20000, Number(url.searchParams.get('radiusM')) || 500));
      const limit = Math.max(1, Math.min(40, Number(url.searchParams.get('limit')) || 24));

      // Snap to a coarse grid so panning the camera reuses one cached answer
      // instead of issuing a new Overpass query per pixel of drift.
      const cellLat = Number(latitude.toFixed(3));
      const cellLon = Number(longitude.toFixed(3));
      const cacheKey = `${cellLat},${cellLon}|${radiusM}|${limit}`;
      const cached = readGeoCache(_placesCache, cacheKey, PLACES_CACHE_MS);
      if (cached) {
        sendGeoJson(res, 200, cached, { cacheSeconds: 300 });
        return;
      }

      try {
        const payload = await dedupeGeoRequest(`places:${cacheKey}`, async () => {
          const ql = buildNearbyPlacesOverpassQL(cellLat, cellLon, radiusM, limit);
          const { elements, stale } = await overpassQueryInternal(ql);
          const places = elements
            .map((element) => normalizeOverpassPlace(element, latitude, longitude))
            .filter(Boolean)
            .sort((a, b) => (
              placeContextPriority(b.types) - placeContextPriority(a.types)
              || a.distanceM - b.distanceM
            ))
            .slice(0, limit);
          return { places, source: 'overpass', stale, error: null };
        });
        writeGeoCache(_placesCache, cacheKey, payload, PLACES_MAX_CACHE);
        sendGeoJson(res, 200, payload, { cacheSeconds: 300 });
      } catch (error) {
        sendGeoJson(res, 502, {
...
/**
 * Rebuild a Nominatim `viewbox` from a client-supplied "west,south,east,north"
 * string, or return null. Never forwards the raw parameter: every component is
 * parsed, range-checked and re-serialized here.
...
const GEV_REALTIME_TOOLS = [
  {
    ...
    parameters: {
      ...
      properties: {
        locationId: {
          type: 'string',
          enum: ['vienna', 'berlin', 'london', 'paris', 'austin', 'sf', 'nyc', 'tokyo', 'dubai', 'dc'],
...
function fetchRegionalPlace(point) {
  // Shares `queueNominatim` with /api/geocode and /api/reverse-geocode: the
  // OSM policy limit is per client, so a private timer here would let three
  // individually-compliant endpoints add up to three requests per second.
  return queueNominatim(async () => {
    ...
    const payload = await fetchRegionalJson(`https://nominatim.openstreetmap.org/reverse?${params}`, {
      headers: {
        'User-Agent': 'JpGodEye/1.0 (+https://github.com/JakobPapaj/jp-god-eye)',
        Referer: 'https://github.com/JakobPapaj/jp-god-eye',
      },
    });
...
async function fetchRegionalNews(place) {
  ...
  try {
    const xml = await fetchRegionalText(`https://news.google.com/rss/search?${rssParams}`, {
      headers: { 'User-Agent': 'JpGodEye/1.0' },
      timeoutMs: 12_000,
    });
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      proxy: {
        // ── OSIRIS ────────────────────────────────────────────────────────
        // The second half of JP GOD EYE: a MapLibre 2D intelligence board
        // running as its own Next.js server. Proxying it here is what makes
        // the pair ONE application from the browser's point of view — one
        // origin, one URL bar, a menu bar that switches between them, and no
        // CORS or third-party-cookie problems between the two halves.
        //
        // Next runs with `basePath: '/osiris'`, so this is a straight
        // pass-through with no path rewriting, and OSIRIS's API routes land on
        // /osiris/api/* where they cannot collide with the globe's own /api/*
        // middleware registered above.
        //
        // `ws: true` carries Next's HMR socket in dev.
        '/osiris': {
          target: `http://localhost:${env.OSIRIS_PORT || 3000}`,
          changeOrigin: true,
          ws: true,
        },
      },
    ...
    define: {
      ...
      // Which keyless imagery source backs the `satellite` map stack. Not a
      // secret — it is a licensing choice the operator makes: the default EOX
      // Sentinel-2 mosaic is CC BY-NC-SA (non-commercial), while `gibs` is
      // U.S. public domain and safe for commercial deployment.
      'import.meta.env.GEV_KEYLESS_IMAGERY': JSON.stringify(env.GEV_KEYLESS_IMAGERY),
    },
    build: {
      rollupOptions: {
        input: {
