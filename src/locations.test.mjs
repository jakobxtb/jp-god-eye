...
// Shaped like one /api/geocode row: OSM data already normalized by the proxy
// into the place-type vocabulary the framing policy speaks.
const AUSTIN_RESULT = {
  name: 'Austin',
  ...
  longitude: -97.7431,
  viewport: {
    low: { latitude: 30.1, longitude: -97.95 },
    high: { latitude: 30.5, longitude: -97.55 },
  },
};
...
async function runSearch(viewer, options, { result = AUSTIN_RESULT, query = 'austin' } = {}) {
  ...
  // No API key is planted: geocoding runs through the keyless /api/geocode
  // proxy, so a search that still demanded a browser-side key would fail here.
  globalThis.window = {};
  globalThis.fetch = async (url) => {
    assert.ok(String(url).startsWith('/api/geocode?'), `unexpected request: ${url}`);
    return { ok: true, json: async () => ({ source: 'nominatim', error: null, results: [result] }) };
  };
  try {
...
//
// EVERY fixture below is SYNTHETIC geometry, constructed here. No captured API
// response appears in this repo — the fixtures assert framing behaviour, not a
// provider's answers, which is what let them survive the move off Google
// geocoding onto the keyless /api/geocode proxy unchanged in meaning.
...
/** Synthetic geocode bounds from explicit south/west/north/east degrees, in the
 *  module-internal {southwest,northeast} shape `placeFramingViewport` consumes. */
...
/** Same box in the {low,high} shape an /api/geocode row carries. */
function proxyViewportOf(bounds) {
  return {
...
/** Build a synthetic /api/geocode row around one of the boxes above. */
function resultOf(types, bounds, lat, lng, label = 'Synthetic Place') {
  return {
    ...
    latitude: lat,
    longitude: lng,
    viewport: proxyViewportOf(bounds),
  };
}
