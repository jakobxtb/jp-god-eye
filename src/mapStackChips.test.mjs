...
/**
 * The chip for a given stack id. Addressing chips by id rather than by row
 * position keeps these assertions pointed at the stack they are actually about
 * when the presentation order changes.
 */
const chipById = (container, stackId) => {
  const chip = container.children.find((child) => child.dataset.stackId === stackId);
  if (!chip) throw new Error(`no chip rendered for stack id: ${stackId}`);
  return chip;
};
...
const CONTROLLER_STACKS = [
  ...
  { id: 'satellite', label: 'Satellite', requiresIon: false, available: true, unavailableReason: null },
  ...
  { id: 'hd-europe', label: 'HD Europe', requiresIon: false, available: true, unavailableReason: null },
...
test('the row renders exactly the six accepted sources', () => {
  ...
  assert.deepEqual(container.children.map(chipText), [
    'Google 3D', 'Satellite', 'Bing Aerial', 'Bing Labels', 'HD Europe', 'OSM',
  ]);
  assert.deepEqual(
    PRESENTED_MAP_STACK_IDS,
    ['photoreal', 'satellite', 'bing-aerial', 'bing-labels', 'hd-europe', 'osm'],
