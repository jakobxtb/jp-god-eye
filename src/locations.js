...
export const CITY_POIS = {
  vienna: {
    name: 'Vienna',
    groundElevation: 170, // meters above WGS84 ellipsoid
    viewBounds: { southwest: { lat: 48.12, lng: 16.18 }, northeast: { lat: 48.32, lng: 16.58 } },
    pois: [
      { name: 'Stephansdom', lat: 48.2085, lon: 16.3735, alt: 520, pitch: -26, heading: 200, buildingHeight: 136 },
      { name: 'Schloss Schönbrunn', lat: 48.1849, lon: 16.3122, alt: 620, pitch: -28, heading: 0, buildingHeight: 25 },
      { name: 'Riesenrad Prater', lat: 48.2166, lon: 16.3958, alt: 480, pitch: -22, heading: 240, buildingHeight: 65 },
      { name: 'Hofburg', lat: 48.2065, lon: 16.3656, alt: 520, pitch: -24, heading: 120, buildingHeight: 30 },
      { name: 'Donauturm', lat: 48.2408, lon: 16.4104, alt: 700, pitch: -20, heading: 210, buildingHeight: 252 },
    ],
  },
  berlin: {
    name: 'Berlin',
    groundElevation: 34,
    viewBounds: { southwest: { lat: 52.44, lng: 13.24 }, northeast: { lat: 52.58, lng: 13.52 } },
    pois: [
      { name: 'Brandenburger Tor', lat: 52.5163, lon: 13.3777, alt: 480, pitch: -25, heading: 90, buildingHeight: 26 },
      { name: 'Fernsehturm', lat: 52.5208, lon: 13.4094, alt: 700, pitch: -20, heading: 220, buildingHeight: 368 },
      { name: 'Reichstag', lat: 52.5186, lon: 13.3762, alt: 500, pitch: -26, heading: 180, buildingHeight: 47 },
    ],
  },
  austin: {
    name: 'Austin',
...
/**
 * Geocode a place name through the keyless `/api/geocode` proxy (Nominatim,
 * with Photon as a real secondary), then fly there at a scale appropriate to
 * the request. Countries and cities use their viewport by default; precise
 * landmarks/buildings use close landmark framing.
 */
export async function searchAndFlyTo(viewer, query, options = {}) {
  ...
  let label = result ? result.displayName : null;
  let types = result?.types || [];
  // The proxy already normalizes the OSM bounding box into the {low,high}
  // shape, which is the same shape the Places recovery returns — so both
  // branches converge on one converter below.
  let viewport = result ? placesViewportToBounds(result.viewport) : null;
