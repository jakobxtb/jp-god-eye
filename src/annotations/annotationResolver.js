...
/**
 * Forward-geocode a place name through the keyless `/api/geocode` proxy
 * (Nominatim, with Photon as a real secondary), biased to the current viewport
 * so "the marina" resolves near where the user is looking.
 *
 * Replaces the metered Google Geocoding call this used to make. The proxy
 * normalizes OSM classifications into the same place-type vocabulary the
 * framing and scope policies already speak, so `types`, `viewport` and
 * `primaryName` keep their existing meanings downstream.
 */
async function geocodePlace(query, biasViewbox, signal) {
  const cacheKey = `${query.toLowerCase()}|${biasViewbox || ''}`;
  const cached = cacheRead(geocodeCache, cacheKey);
  if (cached !== undefined) return cached;

  const params = new URLSearchParams({ q: query, limit: '5' });
  if (biasViewbox) params.set('viewbox', biasViewbox);

  try {
    const response = await fetch(`/api/geocode?${params}`, { signal });
    if (!response.ok) {
      // 429/502 are transient upstream conditions — never poison the cache with
      // a "this place does not exist" answer we did not actually receive.
    ...
    if (!result) {
      // An empty results array IS a definitive not-found, so it is cacheable —
      // unless the proxy reported an error alongside it, which is transient.
      negCache(geocodeCache, cacheKey, signal, !data?.error);
      return null;
    }
    const place = {
      lat: result.latitude,
      lon: result.longitude,
      label: shortLabel(result.displayName),
      // Nominatim states the feature's canonical name directly, so the old
      // heuristic that dug it out of Google's address components is gone.
      primaryName: result.name || shortLabel(result.displayName),
      types: Array.isArray(result.types) ? result.types : [],
      // Already normalized to the {low,high} shape by the proxy.
...
/**
 * View-BOUNDED keyless search for a named landmark/POI near what the operator
 * is looking at.
 *
 * Replaces the metered Google Places Text Search. A plain geocode scatters
 * obscure monument/POI names across the planet; restricting the search to a box
 * around the view centre (`bounded=1`) lands on the ACTUAL feature on screen —
 * the same job the Places call did, through `/api/geocode`.
 *
...
async function placesTextSearch(query, centerLat, centerLon, radiusM, signal) {
  ...
  // Convert the search radius into a degree box around the centre. Longitude
  // degrees shrink toward the poles, so the east/west half-width is scaled by
  // cos(latitude) — without that, a 6 km radius in Reykjavík would span three
  // times the intended ground distance.
  const latHalfDeg = radiusM / 111_320;
  const lonHalfDeg = latHalfDeg / Math.max(0.15, Math.cos((centerLat * Math.PI) / 180));
  const viewbox = [
    (centerLon - lonHalfDeg).toFixed(6),
    (centerLat - latHalfDeg).toFixed(6),
    (centerLon + lonHalfDeg).toFixed(6),
    (centerLat + latHalfDeg).toFixed(6),
  ].join(',');
  ...
  const params = new URLSearchParams({
    ...
    limit: '5',
    viewbox,
    // Hard-bound the search to the box: a nearby-but-unbounded result is
    // exactly the wrong-city hit this recovery exists to prevent.
    bounded: '1',
  ...
  try {
    ...
    const data = await response.json();
    const rows = Array.isArray(data?.places) ? data.places : (Array.isArray(data?.results) ? data.results : []);
    const hit = rows.find((row) => Number.isFinite(row?.latitude) && Number.isFinite(row?.longitude));
    if (!hit) { negCache(placesCache, cacheKey, signal, !data?.error); return null; } // no-match
    const place = {
      lat: hit.latitude,
      lon: hit.longitude,
      label: hit.name || null,
      ...
      // Entity identity/classification: `primaryType`/`types` classify the
      // feature (point-like monument vs compound) and drive disc sizing.
      id: hit.name ? `osm:${hit.category || ''}/${hit.type || ''}` : null,
...
/**
 * Map a geocode result's `types` to a resolution SCOPE so we fetch the right
 * OSM feature at the right size. The proxy normalizes OSM classifications into
 * this vocabulary, so the mapping is unchanged by the move off Google. Country-agnostic: scope only selects the query
...
export function viewportBias(viewer) {
  try {
    ...
    if ([south, west, north, east].some((value) => value === 'NaN')) return null;
    // Nominatim `viewbox` order is west,south,east,north — NOT the
    // "sw|ne" pair Google's `bounds` parameter took. The proxy re-parses and
    // re-serializes this, so a malformed string is dropped rather than
    // forwarded.
