...
// Shaped like the /api/geocode proxy's response: OSM rows already normalized
// into the place-type vocabulary the framing and scope policies speak.
...
/** The view-BOUNDED recovery search shares /api/geocode; `bounded=1` marks it. */
function isBoundedRecoveryCall(url) {
  const value = String(url);
  return value.startsWith('/api/geocode?') && new URL(value, 'http://x').searchParams.get('bounded') === '1';
}

/** The plain forward geocode: same route, no `bounded` flag. */
function isForwardGeocodeCall(url) {
  const value = String(url);
  return value.startsWith('/api/geocode?') && new URL(value, 'http://x').searchParams.get('bounded') !== '1';
}

function installGeocodeMocks(t, handler) {
  const originalWindow = globalThis.window;
  const originalFetch = globalThis.fetch;
  // No API key is planted: geocoding runs through the keyless /api/geocode
  // proxy, so a resolver that still demanded a browser-side key would fail here.
  globalThis.window = {
...
test('ask-side admin bypass: "the Texas Capitol" recovers near-view despite a far state-typed geocode', async (t) => {
  const calls = [];
  installGeocodeMocks(t, async (url) => {
    calls.push(String(url));
    if (isForwardGeocodeCall(url)) {
    ...
    assert.ok(isBoundedRecoveryCall(url), `unexpected request: ${url}`);
    return {
      ok: true,
      json: async () => ({ results: [{
        latitude: 30.2747,
...
test('ask-side admin bypass: explicit "state of Texas" skips recovery and proximity gating', async (t) => {
  ...
  installGeocodeMocks(t, async (url) => {
    calls.push(String(url));
    assert.ok(isForwardGeocodeCall(url), `unexpected request: ${url}`);
    return { ok: true, json: async () => geocodePayload({
      lat: 31.0000,
      lon: -99.0000,
      types: ['administrative_area_level_1', 'political'],
...
test('ask-side admin bypass: admin level 2/3 result types never grant a township bypass', async (t) => {
  ...
  installGeocodeMocks(t, async (url) => {
    calls.push(String(url));
    if (isForwardGeocodeCall(url)) {
      const query = new URL(String(url), 'http://x').searchParams.get('q');
      return { ok: true, json: async () => geocodePayload({
        lat: 39.7817,
        lon: -89.6501,
  ...
  assert.equal(
    calls.filter(isBoundedRecoveryCall).length,
    2,
