...
import { initHazardSimulationPanel } from './hazardSimulationPanel.js';
...
import { MapStackController, DEFAULT_KEYLESS_STACK_ID } from './mapStackController.js';
...
async function init() {
  ...
  try {
    ...
    // app boots on the keyless satellite stack instead of refusing to start —
    // the zero-cost configuration is the supported default, not a degraded one.
    // Geocoding and place search no longer read this key at all; they run
    // through the keyless /api/geocode and /api/places proxies. That also
    // removes the Google Maps ToS coupling that forced Google geocoding on
    // anyone rendering Google tiles (`onlyUsingWithGoogleGeocoder`).
    ...
    const viewer = new Cesium.Viewer('cesiumContainer', {
      ...
      //
      // Cesium defaults this to `true`, which pins the drawing buffer to CSS
      // pixels (pixelRatio = 1) and lets the browser upscale to the panel. On
      // any HiDPI/Retina screen that means the globe, its imagery and every
      // 3D edge are drawn at half linear resolution and stretched — the single
      // largest source of the "everything looks soft" impression, and no
      // amount of sharpening post-process recovers detail that was never
      // rasterized. Turning it off costs fill rate, which is why the ratio is
      // capped below rather than followed blindly.
    ...
    // Cap the effective pixel ratio at 2x. Full DPR on a 3x phone panel means
    // ~9x the fragments of a 1x render for detail the eye cannot resolve at
    // that size, and this app already runs a heavy scene. 2x captures
    // essentially all of the visible sharpening gain.
    const MAX_RENDER_PIXEL_RATIO = 2;
    const nativePixelRatio = Math.max(1, window.devicePixelRatio || 1);
    viewer.resolutionScale = Math.min(MAX_RENDER_PIXEL_RATIO, nativePixelRatio) / nativePixelRatio;

    // Tighter screen-space error = the globe swaps to a higher-detail tile
    // sooner, so imagery and terrain resolve crisply instead of holding a
    // stretched parent tile. 2.0 is Cesium's default; 1.5 is the standard
    // "quality" setting and roughly the point past which extra tile traffic
    // stops buying visible detail.
    ...
    let tileset = null;
    // The `googleApiKey` gate is load-bearing, not just an optimization:
    // `createGooglePhotorealistic3DTileset()` falls back to requesting Google's
    // tiles THROUGH Cesium ion when no Google key is set (verified in the
    // installed Cesium build). On an ion account that would silently spend a
    // metered Google P3DT allowance the free tier does not include, so a
    // keyless install must never reach this call at all.
    if (googleApiKey) {
      loaderStatus.textContent = 'Loading Google 3D Tiles...';
      try {
        // Load Google Photorealistic 3D Tiles.
        //
        // `onlyUsingWithGoogleGeocoder` is deliberately NOT asserted any more:
        // it is a developer acknowledgement that the app geocodes through
        // Google, and this build geocodes through Nominatim. Cesium's one-time
        // console warning is therefore accurate and is left visible — an
        // operator who opts into the metered Google stack is the one who has to
        // reconcile it with Google's Terms of Service (see DATA_SOURCES.md).
      ...
      // No key configured — this is the ordinary zero-cost path, so the globe
      // is simply the one the MapStackController is about to activate. Say
      // nothing about Google here: a missing optional key is not an error.
      loaderStatus.textContent = 'Loading globe imagery...';
      viewer.scene.globe.show = true;
    }
    ...
    const mapStackController = new MapStackController(viewer, {
      ...
      cesiumToken,
      initialStack: tileset ? 'photoreal' : DEFAULT_KEYLESS_STACK_ID,
      // (the controller re-derives its own default below; this is only the
      // pre-construction hint)
    ...
    // Ask the controller which stack this deployment should boot into rather
    // than re-deriving the rule here: it depends on which credentials actually
    // resolved (Google tileset > ion Bing aerial > keyless satellite), and two
    // copies of that decision would drift.
    await mapStackController.setStack(mapStackController.getDefaultStackId(), { silent: true });

    // Initialize the style manager (post-processing, HUD, locations, share links)
    const styleManager = new StyleManager(viewer, { mapStackController });

    // Third view: modelled natural-hazard footprints. Self-contained and
    // inert until the operator opens it, so it costs nothing on boot.
    initHazardSimulationPanel(viewer);
    ...
    if (!styleManager.hasShareState) {
      loaderStatus.textContent = 'Flying to Vienna...';
      flyToHome(viewer);
    } else {
    ...
    console.error("JP GOD EYE initialization failed:", error);
