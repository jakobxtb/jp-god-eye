 ...
 * cables), CC BY-NC-SA (EOX Sentinel-2 cloudless basemap), CC BY-SA (Mapillary),
 * NASA FIRMS, NASA GIBS, CelesTrak, USGS, City of Austin, GBFS operators, OpenSky.
...
export const DATA_CREDITS = [
  ...
  {
    // Every place name, search result and reverse-geocoded label in the app —
    // the location search box, voice fly-to, and annotation targeting — now
    // resolves against OSM instead of a commercial geocoder, so the ODbL
    // attribution covers a far larger surface than the cockpit line above.
    key: 'geocoding-osm',
    html:
      'Geocoding &amp; place search: ' +
      '<a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener">© OpenStreetMap contributors</a> ' +
      'via Nominatim, Photon and Overpass (ODbL 1.0)',
  },
  {
    // CC BY-NC-SA 4.0. The wording follows the attribution the service's own
    // WMTS capabilities document requires, including the linked source.
    key: 'eox-sentinel2-cloudless',
    html:
      'Satellite basemap: Sentinel-2 cloudless 2025 by ' +
      '<a href="https://cloudless.eox.at" target="_blank" rel="noopener">EOX IT Services GmbH</a> ' +
      '(Contains modified Copernicus Sentinel data 2025) — CC BY-NC-SA 4.0, non-commercial use',
  },
  {
    key: 'nasa-gibs',
    html:
      'Satellite basemap fallback: ' +
      '<a href="https://earthdata.nasa.gov/gibs" target="_blank" rel="noopener">NASA EOSDIS GIBS</a> ' +
      '(Blue Marble; U.S. public domain)',
  },
  {
    // Cesium ion assets are only streamed when an operator supplies a token;
    // the credit is registered unconditionally because the popover is a static
    // license index, not a live source indicator.
    key: 'cesium-osm-buildings',
    html:
      '3D buildings (Cesium ion stacks): Cesium OSM Buildings — ' +
      '<a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener">© OpenStreetMap contributors</a> ' +
      '(ODbL 1.0), tiled by Cesium ion',
  },
  {
    key: 'mapillary',
    html:
      'CCTV street-level fallback frames: ' +
      '<a href="https://www.mapillary.com" target="_blank" rel="noopener">Mapillary contributors</a> ' +
      '(CC BY-SA 4.0)',
  },
  {
    key: 'open-meteo',
