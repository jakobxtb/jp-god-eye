...
test('utcMonthKey buckets by UTC month — the period the free tile allowance is granted in', () => {
  assert.equal(utcMonthKey(Date.UTC(2026, 6, 16, 23, 59, 59)), '2026-07');
  assert.equal(utcMonthKey(Date.UTC(2026, 6, 31, 23, 59, 59)), '2026-07');
  // Month rollover is the only boundary that resets the counter now; a day
  // boundary inside the month must NOT.
  assert.equal(utcMonthKey(Date.UTC(2026, 7, 1, 0, 0, 1)), '2026-08');
});

test('normalizeBudget treats the period key as opaque, so day → month migrates by rolling once', () => {
  // A budget file written by the previous day-bucketed governor.
  const legacy = { date: '2026-07-16', count: 39999 };
  const rolled = normalizeBudget(legacy, utcMonthKey(Date.UTC(2026, 6, 16)));
  assert.deepEqual(rolled, { date: '2026-07', count: 0 });
  // And a same-month state then passes through untouched.
  assert.equal(normalizeBudget(rolled, '2026-07'), rolled);
});

test('a day inside the month does not reset the monthly counter', () => {
  const state = { date: '2026-07', count: 120_000 };
  assert.equal(normalizeBudget(state, utcMonthKey(Date.UTC(2026, 6, 28))), state);
  assert.equal(isOverBudget(state, 180_000), false);
  assert.equal(isOverBudget({ date: '2026-07', count: 180_000 }, 180_000), true);
});
