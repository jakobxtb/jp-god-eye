...
const militaryInstallationsLayer = {
  ...
  /**
   * Force a fresh mapped-installation query for the current map view.
   *
   * This used to additionally fire a metered Google Places text search whose
   * results were appended as unreviewed `place_candidate` markers. That
   * augmentation is gone: the free replacement for a Places category search is
   * Overpass, which is already the layer's PRIMARY source — re-asking it under
   * a second name would have added query cost and zero new information. The
   * button still does the real work of refreshing the viewport from OSM.
   */
  searchNearby() {
