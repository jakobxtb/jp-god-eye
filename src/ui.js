...
import { VIEW_MODE_META, nextViewMode, stackIdForViewMode, viewModeForStackId } from './viewModes.js';
...
/**
 * Central UI orchestrator for the JP GOD EYE application.
 *
 * Responsibilities:
...
export class StyleManager {
  ...
  constructor(viewer, { mapStackController = null } = {}) {
    ...
    this._viewModeCycleBtn = document.getElementById('view-mode-cycle');
    this._viewModeLabel = document.getElementById('view-mode-label');
    this._viewMode = 'satellite';
    ...
    this._cctvLightboxImage = document.getElementById('cctv-lightbox-image');
    this._cctvLightboxTitle = document.getElementById('cctv-lightbox-title');
    this._cctvLightboxSource = document.getElementById('cctv-lightbox-source');
    this._cctvLightboxMeta = document.getElementById('cctv-lightbox-meta');
    this._cctvLightboxPrev = document.getElementById('cctv-lightbox-prev');
    this._cctvLightboxNext = document.getElementById('cctv-lightbox-next');
    // Element focus is returned here on close so keyboard users land back on
    // the control they opened the dialog from, not at the top of the document.
  ...
  /**
   * Opens the full-size camera view.
   *
   * The dialog mirrors whatever the panel thumbnail is showing rather than
   * issuing its own request: the frame is already proxied, cached and refreshed
   * by the CCTV layer, so re-fetching it here would double every camera's
   * upstream load for no extra freshness.
   * @returns {void}
   */
  _openCctvLightbox() {
    if (!this._cctvLightbox) return;
    this._cctvLightboxReturnFocus = document.activeElement;
    this._syncCctvLightbox();
    this._cctvLightbox.removeAttribute('hidden');
    // Focus the close control so Escape and Tab behave like a real dialog.
    this._cctvLightboxCloseBtn = this._cctvLightboxCloseBtn
      || document.getElementById('cctv-lightbox-close');
    this._cctvLightboxCloseBtn?.focus();
  }

  /**
   * Closes the full-size camera view and restores focus to its opener.
   * @returns {void}
   */
  _closeCctvLightbox() {
    if (!this._cctvLightbox || this._cctvLightbox.hasAttribute('hidden')) return;
    this._cctvLightbox.setAttribute('hidden', '');
    // Drop the src so a hidden dialog isn't holding a decoded full-size bitmap
    // for every camera the operator ever enlarged.
    this._cctvLightboxImage?.removeAttribute('src');
  ...
  /**
   * Copies the panel's current camera frame and labels into the dialog.
   * Safe to call while closed — it simply primes the next open.
   * @returns {void}
   */
  _syncCctvLightbox() {
    if (!this._cctvLightbox) return;
    const src = this._cctvFrame?.getAttribute('src') || '';
    if (this._cctvLightboxImage) {
      if (src) this._cctvLightboxImage.src = src;
      else this._cctvLightboxImage.removeAttribute('src');
      // The panel thumbnail is decorative (the panel labels it); the enlarged
      // view is the content, so it carries the camera name as its alt text.
      const label = this._cctvSelect?.selectedOptions?.[0]?.textContent?.trim() || 'Camera view';
      this._cctvLightboxImage.alt = label;
    }
    if (this._cctvLightboxTitle) {
      this._cctvLightboxTitle.textContent =
        this._cctvSelect?.selectedOptions?.[0]?.textContent?.trim() || 'CAMERA';
    }
    if (this._cctvLightboxSource) {
      this._cctvLightboxSource.textContent = this._cctvSourceBadge?.textContent?.trim() || '';
    }
    if (this._cctvLightboxMeta) {
      this._cctvLightboxMeta.textContent = this._cctvMeta?.textContent?.trim() || '';
    }
  }
  ...
  _initMapStackControl() {
    ...
    // Seed the cycle from whatever stack is actually live, then let it advance.
    this._viewMode = viewModeForStackId(this.mapStackController.getActiveId());
    this._renderViewModeButton();
    this._viewModeCycleBtn?.addEventListener('click', () => this._cycleViewMode());

    this._renderMapStackState(this.mapStackController.getState());
  }

  /**
   * Advance MAP -> SATELLITE -> 3D and activate the stack behind it.
   * @returns {void}
   */
  _cycleViewMode() {
    if (!this.mapStackController) return;
    const stacks = this.mapStackController.getStacks();
    const next = nextViewMode(this._viewMode, stacks);
    // Nothing serviceable (no stack for any mode) — leave the button alone
    // rather than flipping a label over a map that did not change.
    if (!next) return;
    const stackId = stackIdForViewMode(next, stacks);
    if (!stackId) return;
    this._viewMode = next;
    this._renderViewModeButton();
  ...
  /**
   * Paint the cycle button: current mode, and what the next press will do.
   * @returns {void}
   */
  _renderViewModeButton() {
    if (!this._viewModeCycleBtn) return;
    const stacks = this.mapStackController?.getStacks?.() || [];
    const meta = VIEW_MODE_META[this._viewMode] || VIEW_MODE_META.satellite;
    if (this._viewModeLabel) this._viewModeLabel.textContent = meta.label;
    const icon = this._viewModeCycleBtn.querySelector('.view-mode-icon');
    if (icon) icon.textContent = meta.icon;

    const next = nextViewMode(this._viewMode, stacks);
    const nextEl = this._viewModeCycleBtn.querySelector('.view-mode-next');
    if (nextEl) nextEl.textContent = next ? `→ ${VIEW_MODE_META[next].label}` : '';
    // A deployment that can serve only one mode has nothing to cycle to; say so
    // by disabling rather than by doing nothing on click.
    this._viewModeCycleBtn.disabled = !next || next === this._viewMode;
  }
  ...
  _renderMapStackState(state) {
    ...
    // The chip row can move the stack behind the cycle's back; re-derive the
    // mode from live state so the button never claims the wrong view.
    if (state?.activeId) {
      this._viewMode = viewModeForStackId(state.activeId);
      this._renderViewModeButton();
    }
  ...
  _initAutoHoverPanel(panelId, { openDelayMs = 850, closeDelayMs = 1000 } = {}) {
    ...
    const scheduleClose = () => {
      ...
      closeTimer = window.setTimeout(() => {
        ...
        if (panelEl.classList.contains('collapsed')) return;
        // Never collapse a container that still holds the focused element
        // without first moving focus somewhere usable.
        //
        // The guard above only protects KEYBOARD focus (`:focus-visible`).
        // Focus moved programmatically — which is exactly what the tray does
        // 240 ms after opening, when it hands focus to a Map Source tile — does
        // not match `:focus-visible` in Chromium, so the tray collapses with
        // that tile focused. The tile becomes unreachable, focus falls back to
        // <body>, and every subsequent keystroke goes nowhere: the disclosure
        // is then unreachable by keyboard until the user reaches for the mouse.
    ...
    panelEl.addEventListener('keydown', closeOnEscape);

    // Escape must also close an OPEN tray when focus is no longer inside it.
    //
    // The tray hands focus to a Map Source tile 240 ms after opening, and a
    // tile can be replaced by a chip re-render (a stack switch re-renders the
    // row) — which destroys the focused node and drops focus to <body>. From
    // there the keydown never passes through panelEl, so the listener above
    // never saw the key and the tray stayed open with no keyboard way out:
    // exactly the "stranded disclosure keyboard path" the QA check is named
    // for. Guarded on `contains` so a tray-internal Escape is still handled
    // once, by the panel listener, rather than twice.
    ...
    // Page-lifetime listener, like the other document-level handlers here:
    // the panel it guards is never torn down while the app is running.
  ...
  _initCctvPanel() {
    ...
    this._cctvFrameWrap?.addEventListener('click', () => {
      // Only offer the enlarged view when there is an actual frame to enlarge;
      // opening an empty dialog over the globe would be worse than doing
      // nothing.
      if (!this._cctvFrame?.getAttribute('src')) return;
      this._openCctvLightbox();
    });

    for (const el of document.querySelectorAll('[data-cctv-lightbox-close]')) {
      el.addEventListener('click', () => this._closeCctvLightbox());
    }

    // Escape closes the dialog, and the handler only claims the key while the
    // dialog is actually open so it never swallows Escape from the rest of the
    // app (scene director, panels, tracking release all use it).
    document.addEventListener('keydown', (event) => {
      if (event.key !== 'Escape') return;
      if (this._cctvLightbox?.hasAttribute('hidden')) return;
      event.stopPropagation();
      this._closeCctvLightbox();
    });

    this._cctvLightboxPrev?.addEventListener('click', () => {
      this._runExplicitCctvFocus(
        () => cctvLayer.cycleCamera(-1),
        (cameraId) => cctvLayer.focusCamera(cameraId, 1.4),
      );
    ...
    this._cctvLightboxNext?.addEventListener('click', () => {
      this._runExplicitCctvFocus(
        () => cctvLayer.cycleCamera(1),
        (cameraId) => cctvLayer.focusCamera(cameraId, 1.4),
      );
  ...
  _clearCctvFrame() {
    ...
    if (this._cctvFrame) {
      ...
      // An open enlarged view must not keep showing a camera the panel has
      // already dropped.
      this._syncCctvLightbox?.();
  ...
  _settleCctvFrame(token, src, ok) {
    ...
    // Keep an open enlarged view on the same frame the panel just moved to.
    this._syncCctvLightbox?.();
