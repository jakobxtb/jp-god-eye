...
const STACK_ALIASES = new Map([
  ...
  ['roads', 'osm'],
  ['road map', 'osm'],
  // The keyless satellite stack — what a zero-key install actually boots into,
  // so the words an operator reaches for first have to land somewhere real.
  ['satellite', 'satellite'],
...
async function resolveRadioLocation(args = {}, coordinates = radioCoordinatePair(args), options = {}) {
  ...
  try {
    // Keyless forward geocode (Nominatim via /api/geocode). A geocode that
    // finds nothing returns null, exactly as the Google path did — the caller
    // reports "location not found" rather than guessing a coordinate.
    ...
    return {
      ...
      label: result.displayName || result.name || query,
      country: '',
    };
  } finally {
    clearTimeout(timer);
...
async function reverseGeocode(latitude, longitude) {
  ...
  const request = (async () => {
    try {
      // Keyless reverse geocode (Nominatim via /api/reverse-geocode). The proxy
      // owns the OSM 1 req/s policy, the coordinate-cell cache and the response
      // normalization, so this is a plain read of an already-shaped place.
      //
      // Nominatim answers with ONE feature where Google returned a ranked list,
      // so `labels`/`streetLabels` are shorter here. They are reported at their
      // true length rather than padded — the summarizers that count them must
      // see the real evidence, and nearby place names now come from the richer
      // /api/places/nearby response instead.
      const params = new URLSearchParams({
        lat: String(latitude),
        lon: String(longitude),
      });
      const response = await fetchWithTimeout(`/api/reverse-geocode?${params}`, {}, 8000);
      if (!response.ok) return null;
      const data = await response.json();
      const place = data?.place || null;
      // A definitive "no feature here" (open ocean) is cached; a transient
      // upstream failure is not, so it can succeed on the next look.
      if (!place) {
        if (!data?.error) reverseGeocodeCache.set(key, null);
        return null;
      }
...
async function fetchNearbyPlaces(latitude, longitude, cameraHeightM) {
  ...
  const request = (async () => {
    try {
      ...
      // Overpass-backed keyless place search. Same response contract as the
      // Google Places proxy it replaces, so nothing downstream changes.
      //
      // The timeout is generous because the shapes differ: Google answered a
      // commercial index in well under a second, while a COLD Overpass cell was
      // measured at ~9 s against the live public mirror (and ~29 ms once the
      // proxy has cached it). This is background scene-context enrichment whose
      // failure path is an empty label set, so waiting out a cold cell costs
      // nothing the operator can see, whereas timing out at 8 s would have made
      // the first look at any new place permanently label-less.
      const response = await fetchWithTimeout(`/api/places/nearby?${params}`, {}, 15000);
