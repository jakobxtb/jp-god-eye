...
try {
  ...
  check(
    ...
    JSON.stringify(presentation.ids) === JSON.stringify([
      'photoreal', 'satellite', 'bing-aerial', 'bing-labels', 'hd-europe', 'osm',
  ...
  // Shared by every keyboard test below.
  //
  // The tray opens immediately but hands focus to a Map Source tile 240 ms
  // later, and it auto-dismisses when the pointer is away — which Puppeteer's
  // pointer always is. A fixed 300 ms sleep leaves 60 ms of margin against the
  // focus timer and none against the dismiss timer, which is why these checks
  // failed intermittently on behaviour that was correct. Poll for BOTH facts in
  // one predicate so the snapshot is the instant the condition held.
  const waitOpenWithTileFocus = async () => {
    try {
      const handle = await page.waitForFunction(
        () => {
          ...
          const stackId = document.activeElement?.dataset?.stackId || null;
          return expanded === 'true' && stackId ? { expanded, activeStack: stackId } : null;
        },
  ...
  const keyboardOpen = await waitOpenWithTileFocus();
  check(
  ...
  const keyboardClose = await (async () => {
    try {
      const handle = await page.waitForFunction(() => {
        const expanded = document
          .getElementById('control-panel-toggle')
        ...
        return expanded === 'false' && activeId ? { expanded, activeId } : null;
      }, { timeout: 5000, polling: 'raf' });
      return await handle.jsonValue();
    } catch {
      return await page.evaluate(() => ({
  ...
  const spaceOpen = await waitOpenWithTileFocus();
  check(
  ...
  // Wait on the STATE, never on a fixed delay — but capture it ATOMICALLY.
  //
  // The tray auto-dismisses when the pointer is away from it, and Puppeteer's
  // pointer sits at (0,0) for the whole run. A two-phase "wait for open, then
  // read focus" therefore races that timer: the wait succeeds, the tray closes,
  // and the follow-up read reports a closed tray for behaviour that was
  // correct. Both facts are read inside ONE polled predicate so the snapshot is
  // the instant the condition held, not some later instant.
  const waitCollapsedState = async (want) => {
    try {
      await page.waitForFunction(
        (expected) => document
          .getElementById('control-panel-toggle')
  ...
  await page.keyboard.press('Escape');
  await waitCollapsedState('false');
  // Hold Enter long enough to generate auto-repeat, then release.
  await page.keyboard.down('Enter');
  await new Promise((resolve) => setTimeout(resolve, 320));
  await page.keyboard.up('Enter');
  // The hold may legitimately leave the tray open OR closed depending on how
  // many repeats landed, and the mouse-away timer may close it either way —
  // neither is what this test is about. Force a known closed state, then prove
  // the keyboard path still works from there.
  await page.keyboard.press('Escape');
  await waitCollapsedState('false');
  await page.keyboard.press('Enter');
  const longHoldRecovery = await waitOpenWithTileFocus();
  check(
    'long Enter hold cannot strand the disclosure keyboard path',
    // Assert the PROPERTY the name claims: the path still opens and still hands
    // focus to a map-source tile. Which tile is first is the chip row's own
    // business (and has its own test), so pinning 'photoreal' here only made
    // this break whenever the presented set changed.
    longHoldRecovery.expanded === 'true'
      && typeof longHoldRecovery.activeStack === 'string'
      && longHoldRecovery.activeStack.length > 0,
  ...
  const acquiringLifecycle = await page.evaluate(async () => {
    ...
    // Wait for the notice to REACH a state instead of sleeping a fixed 40 ms.
    // The DOM write is debounced, and on a loaded machine 40 ms was routinely
    // too short — this test failed intermittently on behaviour that was
    // actually correct, just not yet flushed.
    ...
    await settle((v) => v.hidden === false && v.state === 'acquiring' && v.detail === 'SHARED FLIGHT');
    ...
    await settle((v) => v.hidden === false && v.detail === 'SHARED MILITARY FLIGHT');
  ...
  // Retired and unknown stack ids take the SAME path: an unrecognized id falls
  // back to whatever this deployment can actually show, with that tile lit —
  // never onto a hidden source whose status reads ROAD while no tile is pressed.
  //
  // The fallback target is deployment-dependent, which is the point. It is
  // `photoreal` only when a Google key produced a tileset; on the shipped
  // zero-key configuration Google 3D is unavailable, so the fallback is the
  // keyless `satellite` stack. Asserting a hardcoded `photoreal` here would
  // demand that a keyless install strand itself on a stack it can never
  // activate — the exact bug the deployment-aware fallback fixes.
  await page.setViewport({ width: 1000, height: 900, deviceScaleFactor: 1 });
  for (const legacyId of ['bing-road', 'garbage']) {
    ...
    const restored = await page.evaluate(() => {
      ...
      return {
        ...
        // What this deployment SHOULD have fallen back to, derived the same way
        // the controller derives it: Google when its tileset loaded, otherwise
        // the keyless default.
        // Ask the controller for its own default instead of re-deriving the
        // rule here — it now depends on which credentials resolved (Google >
        // ion Bing > keyless satellite), and a second copy of that rule drifts.
        expectedId: window.__godsEyeView.mapStackController.getDefaultStackId(),
    ...
    check(
      `a map=${legacyId} link restores to this deployment's default stack, with that tile lit`,
      restored.activeId === restored.expectedId
        && restored.lastError === null
        && JSON.stringify(restored.pressed) === JSON.stringify([restored.expectedId]),
